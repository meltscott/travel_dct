category_dct = {
        
        'user': {'family' : ['family', 'families', 'kid', 'toddler', 'baby', 'babies', 'teen', 'teenager', 'child', 'children'], 'couples' : ['couple', 'boyfriend', 'girlfriend', 'wife', 'husband', 'fiancee', 'partner'], 'solo' : ['solo', 'single', 'individual'], 'pets' : ['dog', 'pet', 'cat'], 'disabled' : ['disabled', 'handicap', 'accessible'], 'LGBTQI' : ['gay', 'lesbian', 'lgbt', 'lgbtqia', 'lgbtqi'], 'senior' : ['senior', 'pensioner', 'elderly'], 'adult' : ['adult'], 'young' : ['young', '18-30'], 'age' : ['years old'], 'group' : ['group', 'small group', 'guests'], 'veteran' : ['veteran']},

        'topic': {'jet lag': ['jet lag'], 'holiday' : ['holiday', 'trip', 'vacation'], 'staycation' : ['staycation'],'breaks' : ['breaks', 'getaway', 'weekend away'], 'package' : ['package'], 'weather' : ['weather', 'hot', 'cold', 'sun', 'rain'], 'flight' : ['flight', 'airline', 'air fare', 'flying'], 'hotel' : ['hotel'], 'car hire' : ['car rental', 'car hire'], 'car hire advice' : ['damange', 'scratch', 'courtesy car', 'car policy'], 'baggage' : ['baggage', 'luggage', 'bag'], 'places to visit' : ['places to visit', 'places to go', 'where to visit', 'visit'], 'city break' : ['city break'], 'destinations' : ['destinations'], 'top 10' : ['top 10', 'top ten'], 'where to stay' : ['places to stay', 'where to stay', 'area to stay'], 'beaches' : ['beaches'], 'national parks' : ['national parks'], 'gap year' : ['gap year'], 'long haul' : ['long haul'], 'places to eat' : ['places to eat'], 'ski resorts' : ['ski resorts'], 'bucket list' : ['bucket list'], 'attractions' : ['museum', 'attraction', 'shopping', 'sightseeing', 'unesco', 'zoo', 'aquarium', 'architecture', 'archaeology'], 'art' : ['art', 'crafts'], 'beer' : ['beer', 'lager', 'ale'], 'cycling' : ['bike', 'biking', 'cycling', 'cycle'], 'bird_watching' : ['bird',], 'boating' : ['boating', 'sailing', 'boat'], 'sports' : ['bowling', 'bowls', 'baseball', 'basketball', 'soccer', 'football', 'rugby', 'cricket', 'f1', 'roller skate', 'roller skating', 'skateboarding', 'rowing', 'running', 'hockey', 'hunting', 'horse', 'pony'], 'bowling' : ['bowling', 'bowls'], 'cave_exploring' : ['caving'], 'gastronomy' : ['chocolate', 'cheese', 'wine', 'food'], 'churches' : ['church', 'chapel', 'mosque', 'parish', 'temple'], 'countryside' : ['countryside'], 'culture' : ['culture', 'cultural'], 'desert' : ['desert'], 'diving' : ['diving', 'scuba'], 'sea_wildlife' : ['dolphins', 'turtle', 'shark', 'whale', 'seals', 'coral'], 'entertainment' : ['theater', 'musicals', 'opera', 'cinema', 'arcade', 'dancing', 'racing', 'shopping'], 'ferry' : ['ferry'], 'festivals' : ['festivals', 'concerts'], 'fireplace' : ['fire'], 'fishing' : ['fishing'], 'forests' : ['forest', 'jungle', 'trees','woodland'], 'gardens' : ['garden', 'flowers', 'plants', 'herbs', 'terrace'], 'walking' : ['hiking', 'walking'], 'islands' : ['islands'], 'water_sports' : ['water skiing', 'wakeboarding', 'kayaking', 'kitesurfing', 'surfing', 'windsurfing', 'canoe', 'tubing', 'paddle board'], 'library' : ['library'], 'live_music' : ['music', 'jazz', 'piano'], 'monuments' : ['cathedral', 'statue', 'monument', 'landmark', 'ruins'], 'harbour' : ['port', 'harbour', 'marina'], 'public_transport' : ['public transport', 'transport'], 'river' : ['river'], 'safari' : ['safari', 'wildlife'], 'scenery' : ['scenery', 'scenic'], 'tour' : ['tours'], 'extreme_sports' : ['sky diving', 'bungee jump', 'rock climb', 'zip line', 'zip lining', 'rafting'], 'skyscrapers' : ['skyscraper', 'high rise'], 'small_town' : ['small town', 'village'], 'snorkeling' : ['snorkel', 'snorkeling'], 'snow_sports' : ['skiing','ski','snowboarding', 'snow','snowmobile', 'dog sled', 'sledding', 'ice'], 'stargazing' : ['stargazing', 'astrology' ], 'sunny_weather' : ['sunny'], 'sunrise_sunset' : ['sunrise', 'sunset'], 'theme_park' : ['theme park', 'amusement parks', 'fairground'], 'tropical' : ['tropical'], 'university' : ['university', 'college'], 'urban' : ['urban', 'metropolitan'], 'volcanoes' : ['volcano'], 'nature' : ['waterfall', 'jungle', 'trees', 'forest', 'nature', 'natural parks', 'biosphere', 'ecosphere', 'ecological park', 'glaciers', 'mountain', 'lake', 'hot spring', 'northern lights'], 'winery' : ['winery', 'vineyard'], 'yoga' : ['yoga', 'meditation', 'spiritual'], 'wifi' : ['wifi', 'wi-fi', 'internet'], 'swimming' : ['swimming', 'pool', 'private pool', 'swimming pool', 'swim up', 'indoor pool'], 'bath' : ['bath'], 'tennis' : ['tennis'], 'restaurant' : ['restaurant'], 'spa_facilities' : ['sauna', 'steam room', 'hot tub', 'jacuzzi', 'massage', 'wellness'], 'fitness_studio' : ['gym', 'fitness centre', 'fitness', 'fitness center'], 'bar' : ['bar', 'cocktail bar', 'sports bar', 'cocktail'], 'self_catering' : ['self catering', 'self cater', 'self catering', 'kitchen', 'kitchenette', 'cooking facilities', 'cooking', 'microwave'], 'balcony' : ['balcony', 'terrace'], 'air_conditioning' : ['air conditioning', 'ac', 'airconditioning'], 'view' : ['view'], 'breakfast' : ['breakfast', 'breakfast included', 'free breakfast'], 'casino' : ['casino'], 'laundry' : ['washer', 'dryer', 'ironing', 'laundry', 'washing machine'], 'water_park' : ['water park', 'lazy river'], 'suite' : ['suite'], 'penthouse' : ['penthouse'], 'parking' : ['free parking', 'parking'], 'smoking' : ['smoking'], 'non_smoking' : ['non-smoking', 'non smoking', 'no smoking', 'smoke free'], 'café' : ['cafe', 'coffee'], 'childcare' : ['childcare', 'childrens entertainment', 'babysitting', 'baby sitting', 'nanny'], 'concierge' : ['concierge'], 'check-in' : ['late check out', 'check out', 'boarding', 'check in'], 'connecting_rooms' : ['connecting room', 'adjoining room'], 'fine_dining' : ['fine dining'], 'excursions' : ['excursion'], 'fireplace' : ['fireplace', 'open fire'], 'poolside_bar' : ['poolside bar', 'swim up bar'], 'seafood' : ['seafood'], 'sports' : ['sports'], 'central' : ['central', 'centre', 'center'], 'honeymoon' : ['honeymoon', 'mini moon'], 'romantic' : ['romance', 'romantic', 'charming'], 'boutique' : ['small', 'boutique'], 'eco' : ['eco', 'enviroment', 'eco tourism', 'sustainable'], 'resort' : ['resorts'], 'historic' : ['historic', 'history'], 'castle' : ['castle'], 'villa' : ['villa'], '7_star' : ['7 star', 'seven star'], '6_star' : ['6 star', 'six star'], '5_star' : ['5 star', 'five star'], '4_star' : ['4 star', 'five star'], '3_star' : ['3 star', 'five star'], '2_star' : ['2 star', 'five star'], '1_star' : ['1 star', 'five star'], 'all_inclusive' : ['all inclusive', 'full board'], 'half_board' : ['half board'], 'airport' : ['near to airport', 'airport'], 'motel' : ['motel'], 'adventure' : ['adventure', 'activity', 'experience'], 'camping' : ['camping', 'outdoors'], 'country_house' : ['country house'], 'travel' : ['travel', 'places to travel'], 'money' : ['credit card', 'debit card', 'money'], 'insurance' : ['insurance'], 'train' : ['train', 'rail'], 'cancellations and delays' : ['delays', 'cancellation', 'compensation'], 'brexit' : ['brexit'], 'airport lounge' : ['lounge'], 'cruise' : ['cruise'], 'tourism' : ['tourism', 'tourist', 'guides'], 'compare' : ['compare', 'comparison'], 'safety' : ['safe', 'places to avoid'], 'covid-19' : ['quarantine', 'isolate', 'coronavirus', 'covid'], 'news' : ['news'], 'government' : ['government'], 'facts' : ['facts'], 'x_hour_in' : ['hours in'], 'market' : ['market'], 'duty free' : ['duty free'], 'airport allowance' : ['alcohol', 'tobacco', 'cigarettes', 'allowance'], 'motion sickness' : ['motion sickness'], 'bookings' : ['book', 'booking']},
        
        'modifier' : {'best' : ['best', 'top', 'finest', 'leading', 'great', 'favourite', 'beautiful', 'nicest', 'pretty', 'prettiest', 'nice', 'popular'], 'luxury' : ['luxury', 'luxurious', 'expensive', 'exclusive'], 'cheap' : ['cheap', 'affordable', 'budget', 'value', 'bargain',  'under 100', 'under 50', 'low'], 'deals' : ['deal', 'discount', 'promotion', 'codes', 'black friday'], 'last minute' : ['last minute', 'last-minute', 'last min', 'last minute deals'], 'pay_later' : ['pay later'], 'peace and quiet' : ['quiet', 'secluded', 'private', 'relaxing', 'peace', 'hidden', 'unknown', 'secret'], 'lively' : ['lively', 'entertainment'], 'quirky' : ['quirky', 'unusual', 'cool', 'interesting', 'unique', 'trendy', 'coolest', 'trendiest'], 'fun' : ['fun']},

        'dates' : {'valentines' : ['valentine'], 'birthday' : ['birthday'], 'anniversary' : ['anniversary'], 'wedding' : ['wedding'], '4th_july' : ['4th july', 'fourth july', 'independence day'], 'thanks_giving' : ['thanksgiving', 'thanks giving'], 'new_years' : ['new year', 'new years'], 'christmas' : ['christmas', 'xmas'], 'spring break' : ['spring break'], 'january' : ['january'], 'february' : ['february'], 'march' : ['march'], 'april' : ['april'], 'may' : ['may'], 'june' : ['june'], 'july' : ['july'], 'august' : ['august'], 'september' : ['september'], 'october' : ['october'], 'november' : ['november'], 'december' : ['december'], 'winter' : ['winter'], 'summer' : ['summer'], 'spring' : ['spring'], 'autumn' : ['autumn'], 'bank holiday' : ['bank holiday'], 'easter' : ['easter']},

        'brands' : {'american airlines' : ['american airlines'], 'ryanir' : ['ryanir'], 'british airways' : ['ba', 'british airways'], 'easyjet' : ['easyjet'], 'thomas cook' : ['thomas cook'], 'tui' : ['tui'], 'atol' : ['atol'], 'abta' : ['abta'], 'social media' : ['facebook', 'instagram', 'twitter', 'tik tok', 'whatsapp'], 'jet2' : ['jet2'], 'first choice' : ['first choice'], 'enterprise' : ['enterprise'], 'avis' : ['avis'], 'alamo' : ['alamo'], 'hertz' : ['hertz'], 'martin lewis' : ['martin lewis'], 'tripadvisor' : ['tripadvisor'], 'expedia' : ['expedia'], 'google' : ['google'], 'travelzoo' : ['travel zoo', 'travelzoo']},

        'informational' : {'how' : ['how'], 'what' : ['what'], 'where' : ['where'], 'when' : ['when'], 'why' : ['why'], 'can' : ['can'], 'things to do' : ['things to do'], 'will' : ['will'], 'which' : ['which'], 'have' : ['have'], 'should' : ['should'], 'was' : ['was'], 'do' : ['do'], 'is' : ['is'], 'could' : ['could'], 'would' : ['would'], 'advice' : ['advice'], 'tips' : ['tips']},


        'destinations': {'united states of america' : ['united states of america'],
                            'saratoga' : ['saratoga'],
                            'lancashire' : ['lancashire'],
                            'united kingdom' : ['united kingdom'],
                            'london' : ['london'],
                            'edinburgh' : ['edinburgh'],
                            'liverpool' : ['liverpool'],
                            'manchester' : ['manchester'],
                            'mumbai' : ['mumbai'],
                            'dublin' : ['dublin'],
                            'dubai' : ['dubai'],
                            'amsterdam' : ['amsterdam'],
                            'new york' : ['new york', 'ny', 'nyc'],
                            'york' : ['york'],
                            'blackpool' : ['blackpool'],
                            'birmingham' : ['birmingham'],
                            'brighton' : ['brighton'],
                            'glasgow' : ['glasgow'],
                            'bath' : ['bath'],
                            'bournemouth' : ['bournemouth'],
                            'cardiff' : ['cardiff'],
                            'newcastle' : ['newcastle'],
                            'paris' : ['paris'],
                            'bristol' : ['bristol'],
                            'leeds' : ['leeds'],
                            'barcelona' : ['barcelona'],
                            'chester' : ['chester'],
                            'belfast' : ['belfast'],
                            'nottingham' : ['nottingham'],
                            'oxford' : ['oxford'],
                            'rome' : ['rome'],
                            'california' : ['california'],
                            'cambridge' : ['cambridge'],
                            'harrogate' : ['harrogate'],
                            'llandudno' : ['llandudno'],
                            'sheffield' : ['sheffield'],
                            'scarborough' : ['scarborough'],
                            'whitby' : ['whitby'],
                            'the cotswolds' : ['the cotswolds'],
                            'cornwall' : ['cornwall'],
                            'tenerife' : ['tenerife'],
                            'southampton' : ['southampton'],
                            'inverness' : ['inverness'],
                            'cotswolds' : ['cotswolds'],
                            'las vegas' : ['las vegas', 'vegas'],
                            'budapest' : ['budapest'],
                            'prague' : ['prague'],
                            'venice' : ['venice'],
                            'ibiza' : ['ibiza'],
                            'singapore' : ['singapore'],
                            'norwich' : ['norwich'],
                            'windermere' : ['windermere'],
                            'torquay' : ['torquay'],
                            'aberdeen' : ['aberdeen'],
                            'tenby' : ['tenby'],
                            'dorchester' : ['dorchester'],
                            'weymouth' : ['weymouth'],
                            'portsmouth' : ['portsmouth'],
                            'santorini' : ['santorini'],
                            'malta' : ['malta'],
                            'the isle of wight' : ['the isle of wight'],
                            'krakow' : ['krakow'],
                            'copenhagen' : ['copenhagen'],
                            'bali' : ['bali'],
                            'newquay' : ['newquay'],
                            'hastings' : ['hastings'],
                            'reading' : ['reading'],
                            'exeter' : ['exeter'],
                            'lincoln' : ['lincoln'],
                            'plymouth' : ['plymouth'],
                            'windsor' : ['windsor'],
                            'eastbourne' : ['eastbourne'],
                            'stratford upon avon' : ['stratford upon avon'],
                            'isle of wight' : ['isle of wight'],
                            'lisbon' : ['lisbon'],
                            'madrid' : ['madrid'],
                            'cheltenham' : ['cheltenham'],
                            'san francisco' : ['san francisco'],
                            'marrakech' : ['marrakech'],
                            'dundee' : ['dundee'],
                            'jersey' : ['jersey'],
                            'malaga' : ['malaga'],
                            'lanzarote' : ['lanzarote'],
                            'keswick' : ['keswick'],
                            'winchester' : ['winchester'],
                            'fort william' : ['fort william'],
                            'southport' : ['southport'],
                            'canterbury' : ['canterbury'],
                            'st ives' : ['st ives'],
                            'swansea' : ['swansea'],
                            'pitlochry' : ['pitlochry'],
                            'leicester' : ['leicester'],
                            'marbella' : ['marbella'],
                            'croatia' : ['croatia'],
                            'dubrovnik' : ['dubrovnik'],
                            'sorrento' : ['sorrento'],
                            'milan' : ['milan'],
                            'istanbul' : ['istanbul'],
                            'vienna' : ['vienna'],
                            'devon' : ['devon'],
                            'tokyo' : ['tokyo'],
                            'brussels' : ['brussels'],
                            'phuket' : ['phuket'],
                            'albufeira' : ['albufeira'],
                            'st andrews' : ['st andrews'],
                            'bangkok' : ['bangkok'],
                            'miami' : ['miami'],
                            'bruges' : ['bruges'],
                            'normandy' : ['normandy'],
                            'berlin' : ['berlin'],
                            'carlisle' : ['carlisle'],
                            'oban' : ['oban'],
                            'derby' : ['derby'],
                            'st davids' : ['st davids'],
                            'shrewsbury' : ['shrewsbury'],
                            'falmouth' : ['falmouth'],
                            'southend' : ['southend'],
                            'scotland' : ['scotland'],
                            'maldives' : ['maldives'],
                            'paphos' : ['paphos'],
                            'barbados' : ['barbados'],
                            'norfolk' : ['norfolk'],
                            'alicante' : ['alicante'],
                            'seville' : ['seville'],
                            'florence' : ['florence'],
                            'nice' : ['nice'],
                            'grasmere' : ['grasmere'],
                            'lancaster' : ['lancaster'],
                            'chichester' : ['chichester'],
                            'ipswich' : ['ipswich'],
                            'portrush' : ['portrush'],
                            'ambleside' : ['ambleside'],
                            'guildford' : ['guildford'],
                            'margate' : ['margate'],
                            'poole' : ['poole'],
                            'lake garda' : ['lake garda'],
                            'dorset' : ['dorset'],
                            'antalya' : ['antalya'],
                            'menorca' : ['menorca'],
                            'valencia' : ['valencia'],
                            'turkey' : ['turkey'],
                            'east london' : ['east london'],
                            'boston' : ['boston'],
                            'munich' : ['munich'],
                            'corfu' : ['corfu'],
                            'reykjavik' : ['reykjavik'],
                            'hong kong' : ['hong kong'],
                            'geneva' : ['geneva'],
                            'victoria' : ['victoria'],
                            'mykonos' : ['mykonos'],
                            'orlando' : ['orlando'],
                            'spain' : ['spain'],
                            'abu dhabi' : ['abu dhabi'],
                            'iceland' : ['iceland'],
                            'los angeles' : ['los angeles', 'la'],
                            'stirling' : ['stirling'],
                            'galway' : ['galway'],
                            'rye' : ['rye'],
                            'aberystwyth' : ['aberystwyth'],
                            'salisbury' : ['salisbury'],
                            'lyme regis' : ['lyme regis'],
                            'richmond' : ['richmond'],
                            'buxton' : ['buxton'],
                            'chelmsford' : ['chelmsford'],
                            'folkestone' : ['folkestone'],
                            'southwold' : ['southwold'],
                            'colchester' : ['colchester'],
                            'cancun' : ['cancun'],
                            'lake como' : ['lake como'],
                            'toronto' : ['toronto'],
                            'athens' : ['athens'],
                            'kent' : ['kent'],
                            'durban' : ['durban'],
                            'porto' : ['porto'],
                            'st lucia' : ['st lucia'],
                            'wales' : ['wales'],
                            'sydney' : ['sydney'],
                            'luxor' : ['luxor'],
                            'northumberland' : ['northumberland'],
                            'cape town' : ['cape town'],
                            'perth' : ['perth'],
                            'cavtat' : ['cavtat'],
                            'mauritius' : ['mauritius'],
                            'guernsey' : ['guernsey'],
                            'cyprus' : ['cyprus'],
                            'gibraltar' : ['gibraltar'],
                            'padstow' : ['padstow'],
                            'durham' : ['durham'],
                            'alcudia' : ['alcudia'],
                            'salcombe' : ['salcombe'],
                            'newport' : ['newport'],
                            'skipton' : ['skipton'],
                            'penzance' : ['penzance'],
                            'bury st edmunds' : ['bury st edmunds'],
                            'berkeley' : ['berkeley'],
                            'camden' : ['camden'],
                            'sidmouth' : ['sidmouth'],
                            'beaulieu' : ['beaulieu'],
                            'rhodes' : ['rhodes'],
                            'thailand' : ['thailand'],
                            'stockholm' : ['stockholm'],
                            'pennsylvania' : ['pennsylvania'],
                            'split' : ['split'],
                            'the amalfi coast' : ['the amalfi coast'],
                            'france' : ['france'],
                            'st moritz' : ['st moritz'],
                            'naples' : ['naples'],
                            'marmaris' : ['marmaris'],
                            'arran' : ['arran'],
                            'cork' : ['cork'],
                            'northern ireland' : ['northern ireland'],
                            'sardinia' : ['sardinia'],
                            'oslo' : ['oslo'],
                            'monaco' : ['monaco'],
                            'crete' : ['crete'],
                            'greece' : ['greece'],
                            'portugal' : ['portugal'],
                            'vancouver' : ['vancouver'],
                            'taunton' : ['taunton'],
                            'moncao' : ['moncao'],
                            'marlow' : ['marlow'],
                            'koh samui' : ['koh samui'],
                            'glencoe' : ['glencoe'],
                            'hereford' : ['hereford'],
                            'dartmouth' : ['dartmouth'],
                            'dover' : ['dover'],
                            'broadway' : ['broadway'],
                            'ludlow' : ['ludlow'],
                            'darlington' : ['darlington'],
                            'lymington' : ['lymington'],
                            'broadstairs' : ['broadstairs'],
                            'north wales' : ['north wales'],
                            'goa' : ['goa'],
                            'george' : ['george'],
                            'delhi' : ['delhi'],
                            'cheshire' : ['cheshire'],
                            'oludeniz' : ['oludeniz'],
                            'faro' : ['faro'],
                            'antigua' : ['antigua'],
                            'yorkshire' : ['yorkshire'],
                            'derbyshire' : ['derbyshire'],
                            'killarney' : ['killarney'],
                            'zante' : ['zante'],
                            'madeira' : ['madeira'],
                            'ireland' : ['ireland'],
                            'positano' : ['positano'],
                            'essex' : ['essex'],
                            'vermont' : ['vermont'],
                            'skye' : ['skye'],
                            'mexico' : ['mexico'],
                            'algarve' : ['algarve'],
                            'cologne' : ['cologne'],
                            'zurich' : ['zurich'],
                            'hawaii' : ['hawaii'],
                            'san sebastian' : ['san sebastian'],
                            'the seychelles' : ['the seychelles'],
                            'sligo' : ['sligo'],
                            'new hampshire' : ['new hampshire'],
                            'chicago' : ['chicago'],
                            'jamaica' : ['jamaica'],
                            'ayia napa' : ['ayia napa'],
                            'llangollen' : ['llangollen'],
                            'fowey' : ['fowey'],
                            'st mawes' : ['st mawes'],
                            'aldeburgh' : ['aldeburgh'],
                            'gairloch' : ['gairloch'],
                            'st michaels' : ['st michaels'],
                            'nerja' : ['nerja'],
                            'stow on the wold' : ['stow on the wold'],
                            'cirencester' : ['cirencester'],
                            'alnwick' : ['alnwick'],
                            'stamford' : ['stamford'],
                            'newmarket' : ['newmarket'],
                            'seaview' : ['seaview'],
                            'kuala lumpur' : ['kuala lumpur'],
                            'washington dc' : ['washington dc'],
                            'bordeaux' : ['bordeaux'],
                            'bellagio' : ['bellagio'],
                            'surrey' : ['surrey'],
                            'christchurch' : ['christchurch'],
                            'italy' : ['italy'],
                            'isles of scilly' : ['isles of scilly'],
                            'hampshire' : ['hampshire'],
                            'kefalonia' : ['kefalonia'],
                            'conwy' : ['conwy'],
                            'bologna' : ['bologna'],
                            'cairns' : ['cairns'],
                            'sicily' : ['sicily'],
                            'bahamas' : ['bahamas'],
                            'morocco' : ['morocco'],
                            'suffolk' : ['suffolk'],
                            'florida' : ['florida'],
                            'langkawi' : ['langkawi'],
                            'japan' : ['japan'],
                            'sri lanka' : ['sri lanka'],
                            'zanzibar' : ['zanzibar'],
                            'somerset' : ['somerset'],
                            'tel aviv' : ['tel aviv'],
                            'new orleans' : ['new orleans'],
                            'mallorca' : ['mallorca'],
                            'lewis' : ['lewis'],
                            'dornoch' : ['dornoch'],
                            'santa fe' : ['santa fe'],
                            'bude' : ['bude'],
                            'north berwick' : ['north berwick'],
                            'ullapool' : ['ullapool'],
                            'palm beach' : ['palm beach'],
                            'bangor' : ['bangor'],
                            'saundersfoot' : ['saundersfoot'],
                            'grange over sands' : ['grange over sands'],
                            'abergavenny' : ['abergavenny'],
                            'kingston' : ['kingston'],
                            'brecon' : ['brecon'],
                            'santa monica' : ['santa monica'],
                            'riverside' : ['riverside'],
                            'san diego' : ['san diego'],
                            'kingston on' : ['kingston on'],
                            'kilkenny' : ['kilkenny'],
                            'ely' : ['ely'],
                            'ilfracombe' : ['ilfracombe'],
                            'aylesbury' : ['aylesbury'],
                            'truro' : ['truro'],
                            'brockenhurst' : ['brockenhurst'],
                            'ramsgate' : ['ramsgate'],
                            'maidenhead' : ['maidenhead'],
                            'chamonix' : ['chamonix'],
                            'helsinki' : ['helsinki'],
                            'calais' : ['calais'],
                            'pisa' : ['pisa'],
                            'krabi' : ['krabi'],
                            'melbourne' : ['melbourne'],
                            'warsaw' : ['warsaw'],
                            'bilbao' : ['bilbao'],
                            'valletta' : ['valletta'],
                            'verona' : ['verona'],
                            'wellington' : ['wellington'],
                            'bodrum' : ['bodrum'],
                            'hanoi' : ['hanoi'],
                            'cannes' : ['cannes'],
                            'gdansk' : ['gdansk'],
                            'pembrokeshire' : ['pembrokeshire'],
                            'gran canaria' : ['gran canaria'],
                            'hvar' : ['hvar'],
                            'capri' : ['capri'],
                            'nashville' : ['nashville'],
                            'doha' : ['doha'],
                            'seattle' : ['seattle'],
                            'oxfordshire' : ['oxfordshire'],
                            'cuba' : ['cuba'],
                            'ghent' : ['ghent'],
                            'egypt' : ['egypt'],
                            'skiathos' : ['skiathos'],
                            'chiang mai' : ['chiang mai'],
                            'banff' : ['banff'],
                            'rutland' : ['rutland'],
                            'bulgaria' : ['bulgaria'],
                            'kos' : ['kos'],
                            'whitehorse' : ['whitehorse'],
                            'caernarfon' : ['caernarfon'],
                            'yeovil' : ['yeovil'],
                            'rochester' : ['rochester'],
                            'san antonio' : ['san antonio'],
                            'troon' : ['troon'],
                            'burford' : ['burford'],
                            'riviera maya' : ['riviera maya'],
                            'henley on thames' : ['henley on thames'],
                            'st austell' : ['st austell'],
                            'granada' : ['granada'],
                            'cappadocia' : ['cappadocia'],
                            'portofino' : ['portofino'],
                            'luxembourg' : ['luxembourg'],
                            'marseille' : ['marseille'],
                            'riga' : ['riga'],
                            'dusseldorf' : ['dusseldorf'],
                            'manila' : ['manila'],
                            'oman' : ['oman'],
                            'cumbria' : ['cumbria'],
                            'gothenburg' : ['gothenburg'],
                            'moscow' : ['moscow'],
                            'fuerteventura' : ['fuerteventura'],
                            'sharm el sheikh' : ['sharm el sheikh'],
                            'salzburg' : ['salzburg'],
                            'fort lauderdale' : ['fort lauderdale'],
                            'switzerland' : ['switzerland'],
                            'dominican republic' : ['dominican republic'],
                            'bucharest' : ['bucharest'],
                            'tulum' : ['tulum'],
                            'hertfordshire' : ['hertfordshire'],
                            'kiev' : ['kiev'],
                            'brisbane' : ['brisbane'],
                            'kyoto' : ['kyoto'],
                            'rotterdam' : ['rotterdam'],
                            'niagara falls' : ['niagara falls'],
                            'st petersburg' : ['st petersburg'],
                            'monte carlo' : ['monte carlo'],
                            'morzine' : ['morzine'],
                            'chippenham' : ['chippenham'],
                            'punta cana' : ['punta cana'],
                            'havana' : ['havana'],
                            'wells' : ['wells'],
                            'porthmadog' : ['porthmadog'],
                            'santa barbara' : ['santa barbara'],
                            'portland' : ['portland'],
                            'chipping campden' : ['chipping campden'],
                            'helmsley' : ['helmsley'],
                            'nairn' : ['nairn'],
                            'islay' : ['islay'],
                            'dartmoor' : ['dartmoor'],
                            'pickering' : ['pickering'],
                            'st tropez' : ['st tropez'],
                            'larnaca' : ['larnaca'],
                            'basel' : ['basel'],
                            'cadiz' : ['cadiz'],
                            'muscat' : ['muscat'],
                            'indiana' : ['indiana'],
                            'frankfurt' : ['frankfurt'],
                            'sofia' : ['sofia'],
                            'china' : ['china'],
                            'corsica' : ['corsica'],
                            'cairo' : ['cairo'],
                            'limerick' : ['limerick'],
                            'lagos' : ['lagos'],
                            'beijing' : ['beijing'],
                            'grenada' : ['grenada'],
                            'jerusalem' : ['jerusalem'],
                            'shanghai' : ['shanghai'],
                            'bermuda' : ['bermuda'],
                            'fethiye' : ['fethiye'],
                            'aruba' : ['aruba'],
                            'colombo' : ['colombo'],
                            'montenegro' : ['montenegro'],
                            'halkidiki' : ['halkidiki'],
                            'tallinn' : ['tallinn'],
                            'auckland' : ['auckland'],
                            'costa rica' : ['costa rica'],
                            'antwerp' : ['antwerp'],
                            'perthshire' : ['perthshire'],
                            'kalkan' : ['kalkan'],
                            'vietnam' : ['vietnam'],
                            'palermo' : ['palermo'],
                            'interlaken' : ['interlaken'],
                            'limassol' : ['limassol'],
                            'cascais' : ['cascais'],
                            'india' : ['india'],
                            'zermatt' : ['zermatt'],
                            'taormina' : ['taormina'],
                            'lille' : ['lille'],
                            'funchal' : ['funchal'],
                            'jakarta' : ['jakarta'],
                            'bratislava' : ['bratislava'],
                            'londonderry' : ['londonderry'],
                            'frome' : ['frome'],
                            'mahon' : ['mahon'],
                            'honolulu' : ['honolulu'],
                            'bradford on avon' : ['bradford on avon'],
                            'formentera' : ['formentera'],
                            'ibiza town' : ['ibiza town'],
                            'marazion' : ['marazion'],
                            'boracay' : ['boracay'],
                            'montego bay' : ['montego bay'],
                            'angus' : ['angus'],
                            'yarmouth' : ['yarmouth'],
                            'orkney' : ['orkney'],
                            'palm springs' : ['palm springs'],
                            'hamilton' : ['hamilton'],
                            'jesmond' : ['jesmond'],
                            'marlborough' : ['marlborough'],
                            'portstewart' : ['portstewart'],
                            'thurso' : ['thurso'],
                            'woodstock' : ['woodstock'],
                            'bridport' : ['bridport'],
                            'hay on wye' : ['hay on wye'],
                            'ljubljana' : ['ljubljana'],
                            'florida keys' : ['florida keys'],
                            'puerto rico' : ['puerto rico'],
                            'hurghada' : ['hurghada'],
                            'sirmione' : ['sirmione'],
                            'nairobi' : ['nairobi'],
                            'austria' : ['austria'],
                            'fiji' : ['fiji'],
                            'lake louise' : ['lake louise'],
                            'biarritz' : ['biarritz'],
                            'vilnius' : ['vilnius'],
                            'lyon' : ['lyon'],
                            'shropshire' : ['shropshire'],
                            'atlanta' : ['atlanta'],
                            'osaka' : ['osaka'],
                            'germany' : ['germany'],
                            'hoi an' : ['hoi an'],
                            'wiltshire' : ['wiltshire'],
                            'st anton' : ['st anton'],
                            'philadelphia' : ['philadelphia'],
                            'penang' : ['penang'],
                            'warwickshire' : ['warwickshire'],
                            'hua hin' : ['hua hin'],
                            'bergen' : ['bergen'],
                            'wexford' : ['wexford'],
                            'williamsburg' : ['williamsburg'],
                            'waterford' : ['waterford'],
                            'ayrshire' : ['ayrshire'],
                            'belgium' : ['belgium'],
                            'tuscany' : ['tuscany'],
                            'girona' : ['girona'],
                            'jaipur' : ['jaipur'],
                            'whistler' : ['whistler'],
                            'siem reap' : ['siem reap'],
                            'ypres' : ['ypres'],
                            'ras al khaimah' : ['ras al khaimah'],
                            'val thorens' : ['val thorens'],
                            'montreal' : ['montreal'],
                            'aberfeldy' : ['aberfeldy'],
                            'kusadasi' : ['kusadasi'],
                            'phoenix' : ['phoenix'],
                            'mull' : ['mull'],
                            'chatham' : ['chatham'],
                            'portals nous' : ['portals nous'],
                            'melrose' : ['melrose'],
                            'woodbridge' : ['woodbridge'],
                            'fife' : ['fife'],
                            'shaftesbury' : ['shaftesbury'],
                            'weybridge' : ['weybridge'],
                            'lucerne' : ['lucerne'],
                            'qatar' : ['qatar'],
                            'turin' : ['turin'],
                            'toulouse' : ['toulouse'],
                            'beirut' : ['beirut'],
                            'hamburg' : ['hamburg'],
                            'canada' : ['canada'],
                            'verbier' : ['verbier'],
                            'johannesburg' : ['johannesburg'],
                            'reims' : ['reims'],
                            'west sussex' : ['west sussex'],
                            'antibes' : ['antibes'],
                            'houston' : ['houston'],
                            'east sussex' : ['east sussex'],
                            'skopelos' : ['skopelos'],
                            'pula' : ['pula'],
                            'gozo' : ['gozo'],
                            'costa brava' : ['costa brava'],
                            'tobago' : ['tobago'],
                            'tavira' : ['tavira'],
                            'poland' : ['poland'],
                            'nuremberg' : ['nuremberg'],
                            'calgary' : ['calgary'],
                            'turks and caicos' : ['turks and caicos'],
                            'denver' : ['denver'],
                            'lincolnshire' : ['lincolnshire'],
                            'thessaloniki' : ['thessaloniki'],
                            'berkshire' : ['berkshire'],
                            'argyll' : ['argyll'],
                            'gloucestershire' : ['gloucestershire'],
                            'the hague' : ['the hague'],
                            'australia' : ['australia'],
                            'genoa' : ['genoa'],
                            'carmel' : ['carmel'],
                            'chania' : ['chania'],
                            'pollenca' : ['pollenca'],
                            'lavenham' : ['lavenham'],
                            'soller' : ['soller'],
                            'yarm' : ['yarm'],
                            'amalfi' : ['amalfi'],
                            'mlini' : ['mlini'],
                            'sandwich' : ['sandwich'],
                            'helensburgh' : ['helensburgh'],
                            'costa del sol' : ['costa del sol'],
                            'naples florida' : ['naples florida'],
                            'almeria' : ['almeria'],
                            'st agnes' : ['st agnes'],
                            'wareham' : ['wareham'],
                            'udaipur' : ['udaipur'],
                            'naxos' : ['naxos'],
                            'st malo' : ['st malo'],
                            'ile de re' : ['ile de re'],
                            'norway' : ['norway'],
                            'canary islands' : ['canary islands'],
                            'ho chi minh city' : ['ho chi minh city'],
                            'avoriaz' : ['avoriaz'],
                            'big sur' : ['big sur'],
                            'rovinj' : ['rovinj'],
                            'meribel' : ['meribel'],
                            'koh lanta' : ['koh lanta'],
                            'bahrain' : ['bahrain'],
                            'chennai' : ['chennai'],
                            'jura' : ['jura'],
                            'ischia' : ['ischia'],
                            'kitzbuhel' : ['kitzbuhel'],
                            'san jose' : ['san jose'],
                            'carcassonne' : ['carcassonne'],
                            'lake maggiore' : ['lake maggiore'],
                            'la rochelle' : ['la rochelle'],
                            'phnom penh' : ['phnom penh'],
                            'lake geneva' : ['lake geneva'],
                            'mexico city' : ['mexico city'],
                            'amritsar' : ['amritsar'],
                            'malaysia' : ['malaysia'],
                            'azores' : ['azores'],
                            'da nang' : ['da nang'],
                            'st martin' : ['st martin'],
                            'jasper' : ['jasper'],
                            'albany' : ['albany'],
                            'buckinghamshire' : ['buckinghamshire'],
                            'nelson' : ['nelson'],
                            'annecy' : ['annecy'],
                            'cayman islands' : ['cayman islands'],
                            'tampa' : ['tampa'],
                            'philippines' : ['philippines'],
                            'cambodia' : ['cambodia'],
                            'elounda' : ['elounda'],
                            'dunkirk' : ['dunkirk'],
                            'hudson' : ['hudson'],
                            'saint martin' : ['saint martin'],
                            'builth wells' : ['builth wells'],
                            'cowes' : ['cowes'],
                            'dolgellau' : ['dolgellau'],
                            'bushmills' : ['bushmills'],
                            'royal tunbridge wells' : ['royal tunbridge wells'],
                            'narberth' : ['narberth'],
                            'maui' : ['maui'],
                            'les gets' : ['les gets'],
                            'catania' : ['catania'],
                            'ravello' : ['ravello'],
                            'dalyan' : ['dalyan'],
                            'harwich' : ['harwich'],
                            'spean bridge' : ['spean bridge'],
                            'austin' : ['austin'],
                            'st john' : ['st john'],
                            'malmesbury' : ['malmesbury'],
                            'holt' : ['holt'],
                            'devizes' : ['devizes'],
                            'zadar' : ['zadar'],
                            'dallas' : ['dallas'],
                            'kerala' : ['kerala'],
                            'gold coast' : ['gold coast'],
                            'essaouira' : ['essaouira'],
                            'plettenberg bay' : ['plettenberg bay'],
                            'paros' : ['paros'],
                            'memphis' : ['memphis'],
                            'utrecht' : ['utrecht'],
                            'cambridgeshire' : ['cambridgeshire'],
                            'tbilisi' : ['tbilisi'],
                            'palma de mallorca' : ['palma de mallorca'],
                            'puerto vallarta' : ['puerto vallarta'],
                            'avignon' : ['avignon'],
                            'sweden' : ['sweden'],
                            'northamptonshire' : ['northamptonshire'],
                            'worcestershire' : ['worcestershire'],
                            'brazil' : ['brazil'],
                            'leicestershire' : ['leicestershire'],
                            'santander' : ['santander'],
                            'kenya' : ['kenya'],
                            'sliema' : ['sliema'],
                            'zagreb' : ['zagreb'],
                            'belgrade' : ['belgrade'],
                            'andorra' : ['andorra'],
                            'ahmedabad' : ['ahmedabad'],
                            'south africa' : ['south africa'],
                            'tignes' : ['tignes'],
                            'zell am see' : ['zell am see'],
                            'bangalore' : ['bangalore'],
                            'cervinia' : ['cervinia'],
                            'courchevel' : ['courchevel'],
                            'caribbean' : ['caribbean'],
                            'adelaide' : ['adelaide'],
                            'kyrenia' : ['kyrenia'],
                            'strasbourg' : ['strasbourg'],
                            'baltimore' : ['baltimore'],
                            'staffordshire' : ['staffordshire'],
                            'bergamo' : ['bergamo'],
                            'lefkada' : ['lefkada'],
                            'agra' : ['agra'],
                            'tangier' : ['tangier'],
                            'new zealand' : ['new zealand'],
                            'cordoba' : ['cordoba'],
                            'franschhoek' : ['franschhoek'],
                            'parga' : ['parga'],
                            'coll' : ['coll'],
                            'malibu' : ['malibu'],
                            'dumfries and galloway' : ['dumfries and galloway'],
                            'pwllheli' : ['pwllheli'],
                            'cagliari' : ['cagliari'],
                            'valldemossa' : ['valldemossa'],
                            'hersonissos' : ['hersonissos'],
                            'port de pollenca' : ['port de pollenca'],
                            'chester le street' : ['chester le street'],
                            'lucca' : ['lucca'],
                            'altea' : ['altea'],
                            'rock' : ['rock'],
                            'pompeii' : ['pompeii'],
                            'sark' : ['sark'],
                            'banchory' : ['banchory'],
                            'ventnor' : ['ventnor'],
                            'agios nikolaos' : ['agios nikolaos'],
                            'cape cod' : ['cape cod'],
                            'la palma' : ['la palma'],
                            'rethymno' : ['rethymno'],
                            'negril' : ['negril'],
                            'midhurst' : ['midhurst'],
                            'ryde' : ['ryde'],
                            'wadebridge' : ['wadebridge'],
                            'aberdeenshire' : ['aberdeenshire'],
                            'korcula' : ['korcula'],
                            'ottawa' : ['ottawa'],
                            'ukraine' : ['ukraine'],
                            'kandy' : ['kandy'],
                            'sigiriya' : ['sigiriya'],
                            'lima' : ['lima'],
                            'st barts' : ['st barts'],
                            'charleston' : ['charleston'],
                            'finland' : ['finland'],
                            'stellenbosch' : ['stellenbosch'],
                            'jerez' : ['jerez'],
                            'rouen' : ['rouen'],
                            'rio de janeiro' : ['rio de janeiro'],
                            'bedfordshire' : ['bedfordshire'],
                            'izmir' : ['izmir'],
                            'byron bay' : ['byron bay'],
                            'dieppe' : ['dieppe'],
                            'launceston' : ['launceston'],
                            'courmayeur' : ['courmayeur'],
                            'montpellier' : ['montpellier'],
                            'cartagena' : ['cartagena'],
                            'luxembourg city' : ['luxembourg city'],
                            'siena' : ['siena'],
                            'queenstown' : ['queenstown'],
                            'le touquet' : ['le touquet'],
                            'kota kinabalu' : ['kota kinabalu'],
                            'herefordshire' : ['herefordshire'],
                            'versailles' : ['versailles'],
                            'megeve' : ['megeve'],
                            'lombok' : ['lombok'],
                            'hallstatt' : ['hallstatt'],
                            'catalonia' : ['catalonia'],
                            'milos' : ['milos'],
                            'hyderabad' : ['hyderabad'],
                            'new jersey' : ['new jersey'],
                            'santa cruz' : ['santa cruz'],
                            'jordan' : ['jordan'],
                            'alghero' : ['alghero'],
                            'palawan' : ['palawan'],
                            'stockbridge' : ['stockbridge'],
                            'viareggio' : ['viareggio'],
                            'napa valley' : ['napa valley'],
                            'nantes' : ['nantes'],
                            'nasu' : ['nasu'],
                            'linlithgow' : ['linlithgow'],
                            'heraklion' : ['heraklion'],
                            'dingle' : ['dingle'],
                            'mustique' : ['mustique'],
                            'grand cayman' : ['grand cayman'],
                            'hogsback' : ['hogsback'],
                            'ruthin' : ['ruthin'],
                            'nassau' : ['nassau'],
                            'cleveland' : ['cleveland'],
                            'port appin' : ['port appin'],
                            'honfleur' : ['honfleur'],
                            'cefalu' : ['cefalu'],
                            'petworth' : ['petworth'],
                            'redruth' : ['redruth'],
                            'aspen' : ['aspen'],
                            'quebec city' : ['quebec city'],
                            'caen' : ['caen'],
                            'virginia' : ['virginia'],
                            'port douglas' : ['port douglas'],
                            'playa del carmen' : ['playa del carmen'],
                            'dominica' : ['dominica'],
                            'anguilla' : ['anguilla'],
                            'hydra' : ['hydra'],
                            'lviv' : ['lviv'],
                            'russia' : ['russia'],
                            'yokohama' : ['yokohama'],
                            'belize' : ['belize'],
                            'hakone' : ['hakone'],
                            'albania' : ['albania'],
                            'denmark' : ['denmark'],
                            'galapagos islands' : ['galapagos islands'],
                            'guadeloupe' : ['guadeloupe'],
                            'bern' : ['bern'],
                            'marthas vineyard' : ['marthas vineyard'],
                            'saba' : ['saba'],
                            'montana' : ['montana'],
                            'symi' : ['symi'],
                            'hiroshima' : ['hiroshima'],
                            'cusco' : ['cusco'],
                            'svalbard' : ['svalbard'],
                            'okinawa' : ['okinawa'],
                            'yangon' : ['yangon'],
                            'bonaire' : ['bonaire'],
                            'brittany' : ['brittany'],
                            'talpe' : ['talpe'],
                            'grindelwald' : ['grindelwald'],
                            'tanzania' : ['tanzania'],
                            'bogota' : ['bogota'],
                            'canberra' : ['canberra'],
                            'nottinghamshire' : ['nottinghamshire'],
                            'kalamata' : ['kalamata'],
                            'romania' : ['romania'],
                            'hermanus' : ['hermanus'],
                            'cabo san lucas' : ['cabo san lucas'],
                            'israel' : ['israel'],
                            'galle' : ['galle'],
                            'hanover' : ['hanover'],
                            'the netherlands' : ['the netherlands'],
                            'lausanne' : ['lausanne'],
                            'chandigarh' : ['chandigarh'],
                            'indonesia' : ['indonesia'],
                            'montreux' : ['montreux'],
                            'poznan' : ['poznan'],
                            'tofino' : ['tofino'],
                            'kotor' : ['kotor'],
                            'fontainebleau' : ['fontainebleau'],
                            'scottsdale' : ['scottsdale'],
                            'alderney' : ['alderney'],
                            'costa blanca' : ['costa blanca'],
                            'borders' : ['borders'],
                            'madison' : ['madison'],
                            'flaine' : ['flaine'],
                            'mousehole' : ['mousehole'],
                            'county down' : ['county down'],
                            'ocho rios' : ['ocho rios'],
                            'hillsborough' : ['hillsborough'],
                            'gullane' : ['gullane'],
                            'dedham' : ['dedham'],
                            'kolkata' : ['kolkata'],
                            'troyes' : ['troyes'],
                            'sedona' : ['sedona'],
                            'dijon' : ['dijon'],
                            'blandford forum' : ['blandford forum'],
                            'boscastle' : ['boscastle'],
                            'liskeard' : ['liskeard'],
                            'axminster' : ['axminster'],
                            'trieste' : ['trieste'],
                            'lebanon' : ['lebanon'],
                            'slovenia' : ['slovenia'],
                            'varanasi' : ['varanasi'],
                            'arras' : ['arras'],
                            'county durham' : ['county durham'],
                            'wengen' : ['wengen'],
                            'lapland' : ['lapland'],
                            'dortmund' : ['dortmund'],
                            'lake iseo' : ['lake iseo'],
                            'jodhpur' : ['jodhpur'],
                            'lecce' : ['lecce'],
                            'peru' : ['peru'],
                            'easter island' : ['easter island'],
                            'epernay' : ['epernay'],
                            'bentota' : ['bentota'],
                            'santiago' : ['santiago'],
                            'detroit' : ['detroit'],
                            'sintra' : ['sintra'],
                            'shimla' : ['shimla'],
                            'puglia' : ['puglia'],
                            'savannah' : ['savannah'],
                            'hobart' : ['hobart'],
                            'vis' : ['vis'],
                            'koh rong' : ['koh rong'],
                            'toledo' : ['toledo'],
                            'alexandria' : ['alexandria'],
                            'diani beach' : ['diani beach'],
                            'luang prabang' : ['luang prabang'],
                            'sao paulo' : ['sao paulo'],
                            'deauville' : ['deauville'],
                            'rimini' : ['rimini'],
                            'panama' : ['panama'],
                            'holbox' : ['holbox'],
                            'texas' : ['texas'],
                            'bayeux' : ['bayeux'],
                            'val disere' : ['val disere'],
                            'perpignan' : ['perpignan'],
                            'roscoff' : ['roscoff'],
                            'darwin' : ['darwin'],
                            'liege' : ['liege'],
                            'nicosia' : ['nicosia'],
                            'kochi' : ['kochi'],
                            'st louis' : ['st louis'],
                            'sierra nevada' : ['sierra nevada'],
                            'provence' : ['provence'],
                            'napa' : ['napa'],
                            'east lothian' : ['east lothian'],
                            'tremezzo' : ['tremezzo'],
                            'litchfield' : ['litchfield'],
                            'le mans' : ['le mans'],
                            'gocek' : ['gocek'],
                            'zaragoza' : ['zaragoza'],
                            'juan les pins' : ['juan les pins'],
                            'providence' : ['providence'],
                            'stresa' : ['stresa'],
                            'menton' : ['menton'],
                            'oia' : ['oia'],
                            'milford' : ['milford'],
                            'olbia' : ['olbia'],
                            'lexington' : ['lexington'],
                            'st kitts' : ['st kitts'],
                            'aboyne' : ['aboyne'],
                            'colorado springs' : ['colorado springs'],
                            'bergerac' : ['bergerac'],
                            'st augustine' : ['st augustine'],
                            'long island' : ['long island'],
                            'chagford' : ['chagford'],
                            'colombia' : ['colombia'],
                            'yucatan' : ['yucatan'],
                            'panama city' : ['panama city'],
                            'st vincent and the grenadines' : ['st vincent and the grenadines'],
                            'martinique' : ['martinique'],
                            'salamanca' : ['salamanca'],
                            'ushuaia' : ['ushuaia'],
                            'spetses' : ['spetses'],
                            'british virgin islands' : ['british virgin islands'],
                            'pune' : ['pune'],
                            'rotorua' : ['rotorua'],
                            'ronda' : ['ronda'],
                            'rhode island' : ['rhode island'],
                            'lugano' : ['lugano'],
                            'ethiopia' : ['ethiopia'],
                            'port elizabeth' : ['port elizabeth'],
                            'andaman islands' : ['andaman islands'],
                            'pittsburgh' : ['pittsburgh'],
                            'san gimignano' : ['san gimignano'],
                            'cozumel' : ['cozumel'],
                            'bhutan' : ['bhutan'],
                            'buenos aires' : ['buenos aires'],
                            'tarifa' : ['tarifa'],
                            'gstaad' : ['gstaad'],
                            'sifnos' : ['sifnos'],
                            'laos' : ['laos'],
                            'tarragona' : ['tarragona'],
                            'khao lak' : ['khao lak'],
                            'cortina' : ['cortina'],
                            'innsbruck' : ['innsbruck'],
                            'county donegal' : ['county donegal'],
                            'arizona' : ['arizona'],
                            'beaune' : ['beaune'],
                            'victoria bc' : ['victoria bc'],
                            'olhao' : ['olhao'],
                            'syracuse' : ['syracuse'],
                            'mandalay' : ['mandalay'],
                            'nuwara eliya' : ['nuwara eliya'],
                            'bavaria' : ['bavaria'],
                            'quito' : ['quito'],
                            'rabat' : ['rabat'],
                            'alanya' : ['alanya'],
                            'alice springs' : ['alice springs'],
                            'ios' : ['ios'],
                            'treviso' : ['treviso'],
                            'tucson' : ['tucson'],
                            'nevis' : ['nevis'],
                            'indianapolis' : ['indianapolis'],
                            'bardolino' : ['bardolino'],
                            'cherbourg' : ['cherbourg'],
                            'chattanooga' : ['chattanooga'],
                            'albuquerque' : ['albuquerque'],
                            'cassis' : ['cassis'],
                            'canmore' : ['canmore'],
                            'malcesine' : ['malcesine'],
                            'rapallo' : ['rapallo'],
                            'ciutadella' : ['ciutadella'],
                            'imerovigli' : ['imerovigli'],
                            'kenmare' : ['kenmare'],
                            'sacramento' : ['sacramento'],
                            'dinard' : ['dinard'],
                            'san juan' : ['san juan'],
                            'minneapolis' : ['minneapolis'],
                            'shipston on stour' : ['shipston on stour'],
                            'county kerry' : ['county kerry'],
                            'delft' : ['delft'],
                            'michigan' : ['michigan'],
                            'latvia' : ['latvia'],
                            'hungary' : ['hungary'],
                            'ithaca' : ['ithaca'],
                            'county meath' : ['county meath'],
                            'perugia' : ['perugia'],
                            'st emilion' : ['st emilion'],
                            'alba' : ['alba'],
                            'tipperary' : ['tipperary'],
                            'halifax ns' : ['halifax ns'],
                            'colorado' : ['colorado'],
                            'maastricht' : ['maastricht'],
                            'lithuania' : ['lithuania'],
                            'lake orta' : ['lake orta'],
                            'boulogne' : ['boulogne'],
                            'jackson hole' : ['jackson hole'],
                            'united arab emirates' : ['united arab emirates'],
                            'folegandros' : ['folegandros'],
                            'channel islands' : ['channel islands'],
                            'koh yao noi' : ['koh yao noi'],
                            'curacao' : ['curacao'],
                            'estonia' : ['estonia'],
                            'massachusetts' : ['massachusetts'],
                            'salalah' : ['salalah'],
                            'loire valley' : ['loire valley'],
                            'chiang rai' : ['chiang rai'],
                            'zambia' : ['zambia'],
                            'hue' : ['hue'],
                            'chengdu' : ['chengdu'],
                            'sagres' : ['sagres'],
                            'kas' : ['kas'],
                            'mozambique' : ['mozambique'],
                            'trogir' : ['trogir'],
                            'tasmania' : ['tasmania'],
                            'stuttgart' : ['stuttgart'],
                            'koufonisia' : ['koufonisia'],
                            'slovakia' : ['slovakia'],
                            'plovdiv' : ['plovdiv'],
                            'matera' : ['matera'],
                            'darjeeling' : ['darjeeling'],
                            'pissouri' : ['pissouri'],
                            'st kitts and nevis' : ['st kitts and nevis'],
                            'cameron highlands' : ['cameron highlands'],
                            'aarhus' : ['aarhus'],
                            'andalucia' : ['andalucia'],
                            'sarajevo' : ['sarajevo'],
                            'rajasthan' : ['rajasthan'],
                            'trondheim' : ['trondheim'],
                            'sapporo' : ['sapporo'],
                            'vientiane' : ['vientiane'],
                            'kanchanaburi' : ['kanchanaburi'],
                            'grenoble' : ['grenoble'],
                            'medellin' : ['medellin'],
                            'jaffna' : ['jaffna'],
                            'nantucket' : ['nantucket'],
                            'argentina' : ['argentina'],
                            'orleans' : ['orleans'],
                            'arcachon' : ['arcachon'],
                            'pamplona' : ['pamplona'],
                            'gurgaon' : ['gurgaon'],
                            'thiruvananthapuram' : ['thiruvananthapuram'],
                            'arles' : ['arles'],
                            'amiens' : ['amiens'],
                            'sutherland' : ['sutherland'],
                            'eze' : ['eze'],
                            'st helena' : ['st helena'],
                            'milwaukee' : ['milwaukee'],
                            'shetland islands' : ['shetland islands'],
                            'st paul de vence' : ['st paul de vence'],
                            'lanarkshire' : ['lanarkshire'],
                            'brac' : ['brac'],
                            'montauk' : ['montauk'],
                            'saint paul de vence' : ['saint paul de vence'],
                            'harris' : ['harris'],
                            'varenna' : ['varenna'],
                            'sestriere' : ['sestriere'],
                            'kansas city' : ['kansas city'],
                            'braga' : ['braga'],
                            'rennes' : ['rennes'],
                            'ooty' : ['ooty'],
                            'alacati' : ['alacati'],
                            'garda' : ['garda'],
                            'beziers' : ['beziers'],
                            'park city' : ['park city'],
                            'oita' : ['oita'],
                            'les arcs' : ['les arcs'],
                            'orange' : ['orange'],
                            'salina' : ['salina'],
                            'san luis obispo' : ['san luis obispo'],
                            'knysna' : ['knysna'],
                            'buckfastleigh' : ['buckfastleigh'],
                            'antigua and barbuda' : ['antigua and barbuda'],
                            'nimes' : ['nimes'],
                            'utah' : ['utah'],
                            'kazakhstan' : ['kazakhstan'],
                            'georgia' : ['georgia'],
                            'sihanoukville' : ['sihanoukville'],
                            'obergurgl' : ['obergurgl'],
                            'aix en provence' : ['aix en provence'],
                            'blenheim' : ['blenheim'],
                            'baden baden' : ['baden baden'],
                            'namibia' : ['namibia'],
                            'hatton' : ['hatton'],
                            'rishikesh' : ['rishikesh'],
                            'podgorica' : ['podgorica'],
                            'fukuoka' : ['fukuoka'],
                            'istria' : ['istria'],
                            'sibenik' : ['sibenik'],
                            'nara' : ['nara'],
                            'bremen' : ['bremen'],
                            'lombardy' : ['lombardy'],
                            'cincinnati' : ['cincinnati'],
                            'torres del paine' : ['torres del paine'],
                            'dahab' : ['dahab'],
                            'la spezia' : ['la spezia'],
                            'trincomalee' : ['trincomalee'],
                            'fogo island' : ['fogo island'],
                            'hangzhou' : ['hangzhou'],
                            'bintan' : ['bintan'],
                            'bled' : ['bled'],
                            'mons' : ['mons'],
                            'parma' : ['parma'],
                            'lopud' : ['lopud'],
                            'jaisalmer' : ['jaisalmer'],
                            'mysore' : ['mysore'],
                            'fremantle' : ['fremantle'],
                            'mendoza' : ['mendoza'],
                            'myanmar' : ['myanmar'],
                            'a coruna' : ['a coruna'],
                            'kovalam' : ['kovalam'],
                            'ostuni' : ['ostuni'],
                            'puri' : ['puri'],
                            'chile' : ['chile'],
                            'pretoria' : ['pretoria'],
                            'tivat' : ['tivat'],
                            'czech republic' : ['czech republic'],
                            'peloponnese' : ['peloponnese'],
                            'serbia' : ['serbia'],
                            'almaty' : ['almaty'],
                            'mussoorie' : ['mussoorie'],
                            'sausalito' : ['sausalito'],
                            'riva del garda' : ['riva del garda'],
                            'moab' : ['moab'],
                            'yountville' : ['yountville'],
                            'mahe' : ['mahe'],
                            'prescott' : ['prescott'],
                            'livigno' : ['livigno'],
                            'como' : ['como'],
                            'dunedin' : ['dunedin'],
                            'jacksonville' : ['jacksonville'],
                            'bormio' : ['bormio'],
                            'cadaques' : ['cadaques'],
                            'oklahoma city' : ['oklahoma city'],
                            'wilderness' : ['wilderness'],
                            'bequia' : ['bequia'],
                            'west lothian' : ['west lothian'],
                            'sonoma' : ['sonoma'],
                            'alencon' : ['alencon'],
                            'glamorgan' : ['glamorgan'],
                            'saint raphael' : ['saint raphael'],
                            'collioure' : ['collioure'],
                            'carmarthenshire' : ['carmarthenshire'],
                            'modena' : ['modena'],
                            'buffalo' : ['buffalo'],
                            'powys' : ['powys'],
                            'tropea' : ['tropea'],
                            'oaxaca' : ['oaxaca'],
                            'beaminster' : ['beaminster'],
                            'crediton' : ['crediton'],
                            'acapulco' : ['acapulco'],
                            'burgundy' : ['burgundy'],
                            'bolivia' : ['bolivia'],
                            'syros' : ['syros'],
                            'newfoundland' : ['newfoundland'],
                            'marsa alam' : ['marsa alam'],
                            'sveti stefan' : ['sveti stefan'],
                            'wanaka' : ['wanaka'],
                            'nicaragua' : ['nicaragua'],
                            'missouri' : ['missouri'],
                            'gwynedd' : ['gwynedd'],
                            'sao tome and principe' : ['sao tome and principe'],
                            'pyrenees' : ['pyrenees'],
                            'dresden' : ['dresden'],
                            'poitiers' : ['poitiers'],
                            'cinque terre' : ['cinque terre'],
                            'chartres' : ['chartres'],
                            'crans montana' : ['crans montana'],
                            'connecticut' : ['connecticut'],
                            'bagan' : ['bagan'],
                            'oregon' : ['oregon'],
                            'stowe' : ['stowe'],
                            'maryland' : ['maryland'],
                            'lake placid' : ['lake placid'],
                            'pemba island' : ['pemba island'],
                            'ecuador' : ['ecuador'],
                            'santiago de compostela' : ['santiago de compostela'],
                            'lodz' : ['lodz'],
                            'south carolina' : ['south carolina'],
                            'north carolina' : ['north carolina'],
                            'sabah' : ['sabah'],
                            'uruguay' : ['uruguay'],
                            'guatemala' : ['guatemala'],
                            'nafplio' : ['nafplio'],
                            'serifos' : ['serifos'],
                            'ontario' : ['ontario'],
                            'annapolis' : ['annapolis'],
                            'preveza' : ['preveza'],
                            'alesund' : ['alesund'],
                            'kanazawa' : ['kanazawa'],
                            'whitsunday islands' : ['whitsunday islands'],
                            'queensland' : ['queensland'],
                            'leipzig' : ['leipzig'],
                            'begur' : ['begur'],
                            'paxos' : ['paxos'],
                            'burgos' : ['burgos'],
                            'mayrhofen' : ['mayrhofen'],
                            'vouliagmeni' : ['vouliagmeni'],
                            'kaikoura' : ['kaikoura'],
                            'asheville' : ['asheville'],
                            'st thomas' : ['st thomas'],
                            'staunton' : ['staunton'],
                            'bolzano' : ['bolzano'],
                            'south lake tahoe' : ['south lake tahoe'],
                            'monmouthshire' : ['monmouthshire'],
                            'trinidad cuba' : ['trinidad cuba'],
                            'manali' : ['manali'],
                            'dinan' : ['dinan'],
                            'machrihanish' : ['machrihanish'],
                            'phu quoc' : ['phu quoc'],
                            'umhlanga' : ['umhlanga'],
                            'louisville' : ['louisville'],
                            'moray' : ['moray'],
                            'catalina island' : ['catalina island'],
                            'wilmington' : ['wilmington'],
                            'sissi' : ['sissi'],
                            'stirlingshire' : ['stirlingshire'],
                            'daylesford' : ['daylesford'],
                            'otranto' : ['otranto'],
                            'goreme' : ['goreme'],
                            'abbeville' : ['abbeville'],
                            'guadalajara' : ['guadalajara'],
                            'bay of islands' : ['bay of islands'],
                            'amorgos' : ['amorgos'],
                            'margaret river' : ['margaret river'],
                            'tennessee' : ['tennessee'],
                            'kobe' : ['kobe'],
                            'alonissos' : ['alonissos'],
                            'dambulla' : ['dambulla'],
                            'barbuda' : ['barbuda'],
                            'astana' : ['astana'],
                            'hampi' : ['hampi'],
                            'louisiana' : ['louisiana'],
                            'county cork' : ['county cork'],
                            'mui ne' : ['mui ne'],
                            'tinos' : ['tinos'],
                            'mont tremblant' : ['mont tremblant'],
                            'mostar' : ['mostar'],
                            'windhoek' : ['windhoek'],
                            'armenia' : ['armenia'],
                            'mljet' : ['mljet'],
                            'patmos' : ['patmos'],
                            'wisconsin' : ['wisconsin'],
                            'champagne' : ['champagne'],
                            'ohio' : ['ohio'],
                            'te anau' : ['te anau'],
                            'varkala' : ['varkala'],
                            'malacca' : ['malacca'],
                            'kangaroo island' : ['kangaroo island'],
                            'winnipeg' : ['winnipeg'],
                            'coimbra' : ['coimbra'],
                            'nanjing' : ['nanjing'],
                            'lech' : ['lech'],
                            'minnesota' : ['minnesota'],
                            'delaware' : ['delaware'],
                            'chios' : ['chios'],
                            'fez' : ['fez'],
                            'andros' : ['andros'],
                            'evora' : ['evora'],
                            'haridwar' : ['haridwar'],
                            'ipoh' : ['ipoh'],
                            'segovia' : ['segovia'],
                            'ishigaki' : ['ishigaki'],
                            'san agustin' : ['san agustin'],
                            'honduras' : ['honduras'],
                            'suzhou' : ['suzhou'],
                            'munster' : ['munster'],
                            'lucknow' : ['lucknow'],
                            'nova scotia' : ['nova scotia'],
                            'lubeck' : ['lubeck'],
                            'vail' : ['vail'],
                            'sapa' : ['sapa'],
                            'vigo' : ['vigo'],
                            'calabria' : ['calabria'],
                            'dordogne' : ['dordogne'],
                            'portsmouth nh' : ['portsmouth nh'],
                            'santa rosa' : ['santa rosa'],
                            'kennebunkport' : ['kennebunkport'],
                            'herm' : ['herm'],
                            'prince albert' : ['prince albert'],
                            'ancona' : ['ancona'],
                            'omaha' : ['omaha'],
                            'praslin' : ['praslin'],
                            'la thuile' : ['la thuile'],
                            'oviedo' : ['oviedo'],
                            'bourg st maurice' : ['bourg st maurice'],
                            'lonavala' : ['lonavala'],
                            'calvi' : ['calvi'],
                            'logrono' : ['logrono'],
                            'valladolid' : ['valladolid'],
                            'saumur' : ['saumur'],
                            'tortola' : ['tortola'],
                            'bourges' : ['bourges'],
                            'ravenna' : ['ravenna'],
                            'torrox' : ['torrox'],
                            'amboise' : ['amboise'],
                            'saratoga springs' : ['saratoga springs'],
                            'monopoli' : ['monopoli'],
                            'niseko' : ['niseko'],
                            'guimaraes' : ['guimaraes'],
                            'healdsburg' : ['healdsburg'],
                            'exumas' : ['exumas'],
                            'paso robles' : ['paso robles'],
                            'ajaccio' : ['ajaccio'],
                            'la gomera' : ['la gomera'],
                            'takayama' : ['takayama'],
                            'saint jean cap ferrat' : ['saint jean cap ferrat'],
                            'st jean de luz' : ['st jean de luz'],
                            'caithness' : ['caithness'],
                            'monza' : ['monza'],
                            'county antrim' : ['county antrim'],
                            'chiddingfold' : ['chiddingfold'],
                            'alberta' : ['alberta'],
                            'monemvasia' : ['monemvasia'],
                            'pasikudah' : ['pasikudah'],
                            'heidelberg' : ['heidelberg'],
                            'buzios' : ['buzios'],
                            'broome' : ['broome'],
                            'kranjska gora' : ['kranjska gora'],
                            'us virgin islands' : ['us virgin islands'],
                            'little rock' : ['little rock'],
                            'aachen' : ['aachen'],
                            'st martin de belleville' : ['st martin de belleville'],
                            'gangtok' : ['gangtok'],
                            'bariloche' : ['bariloche'],
                            'kentucky' : ['kentucky'],
                            'piedmont' : ['piedmont'],
                            'napier' : ['napier'],
                            'new mexico' : ['new mexico'],
                            'prince edward island' : ['prince edward island'],
                            'british columbia' : ['british columbia'],
                            'umbria' : ['umbria'],
                            'galicia' : ['galicia'],
                            'wyoming' : ['wyoming'],
                            'assisi' : ['assisi'],
                            'taghazout' : ['taghazout'],
                            'asturias' : ['asturias'],
                            'picardy' : ['picardy'],
                            'mossel bay' : ['mossel bay'],
                            'murren' : ['murren'],
                            'sikkim' : ['sikkim'],
                            'dharamshala' : ['dharamshala'],
                            'quintana roo' : ['quintana roo'],
                            'procida' : ['procida'],
                            'thekkady' : ['thekkady'],
                            'seefeld' : ['seefeld'],
                            'paraguay' : ['paraguay'],
                            'fatima' : ['fatima'],
                            'cognac' : ['cognac'],
                            'essen' : ['essen'],
                            'flores' : ['flores'],
                            'engelberg' : ['engelberg'],
                            'nha trang' : ['nha trang'],
                            'grasse' : ['grasse'],
                            'ischgl' : ['ischgl'],
                            'antigua guatemala' : ['antigua guatemala'],
                            'lamu' : ['lamu'],
                            'punjab' : ['punjab'],
                            'balearic islands' : ['balearic islands'],
                            'vevey' : ['vevey'],
                            'montevideo' : ['montevideo'],
                            'bonn' : ['bonn'],
                            'zamora' : ['zamora'],
                            'midlothian' : ['midlothian'],
                            'bloomington' : ['bloomington'],
                            'beppu' : ['beppu'],
                            'montepulciano' : ['montepulciano'],
                            'charlottesville' : ['charlottesville'],
                            'frejus' : ['frejus'],
                            'chantilly' : ['chantilly'],
                            'le havre' : ['le havre'],
                            'matheran' : ['matheran'],
                            'boise' : ['boise'],
                            'delray beach' : ['delray beach'],
                            'pau' : ['pau'],
                            'lake lugano' : ['lake lugano'],
                            'trapani' : ['trapani'],
                            'costa de la luz' : ['costa de la luz'],
                            'grand bahama' : ['grand bahama'],
                            'la clusaz' : ['la clusaz'],
                            'colmar' : ['colmar'],
                            'angers' : ['angers'],
                            'nainital' : ['nainital'],
                            'chambery' : ['chambery'],
                            'la plagne' : ['la plagne'],
                            'county cavan' : ['county cavan'],
                            'island of hawaii' : ['island of hawaii'],
                            'vicenza' : ['vicenza'],
                            'thira' : ['thira'],
                            'dehradun' : ['dehradun'],
                            'ceredigion' : ['ceredigion'],
                            'provincetown' : ['provincetown'],
                            'narbonne' : ['narbonne'],
                            'bar harbor' : ['bar harbor'],
                            'ericeira' : ['ericeira'],
                            'salvador' : ['salvador'],
                            'bosnia and herzegovina' : ['bosnia and herzegovina'],
                            'western australia' : ['western australia'],
                            'veneto' : ['veneto'],
                            'caceres' : ['caceres'],
                            'leuven' : ['leuven'],
                            'ouarzazate' : ['ouarzazate'],
                            'sarlat' : ['sarlat'],
                            'perast' : ['perast'],
                            'naoshima' : ['naoshima'],
                            'ikaria' : ['ikaria'],
                            'new brunswick' : ['new brunswick'],
                            'county mayo' : ['county mayo'],
                            'ayutthaya' : ['ayutthaya'],
                            'new south wales' : ['new south wales'],
                            'ko samui' : ['ko samui'],
                            'puerto escondido' : ['puerto escondido'],
                            'oudtshoorn' : ['oudtshoorn'],
                            'arequipa' : ['arequipa'],
                            'mississippi' : ['mississippi'],
                            'kumamoto' : ['kumamoto'],
                            'puebla' : ['puebla'],
                            'county clare' : ['county clare'],
                            'calcutta' : ['calcutta'],
                            'setubal' : ['setubal'],
                            'java' : ['java'],
                            'pantelleria' : ['pantelleria'],
                            'valparaiso' : ['valparaiso'],
                            'herceg novi' : ['herceg novi'],
                            'namur' : ['namur'],
                            'nagano' : ['nagano'],
                            'kavala' : ['kavala'],
                            'tauranga' : ['tauranga'],
                            'cuenca' : ['cuenca'],
                            'yangshuo' : ['yangshuo'],
                            'cannon beach' : ['cannon beach'],
                            'lazio' : ['lazio'],
                            'tamarindo' : ['tamarindo'],
                            'giverny' : ['giverny'],
                            'arkansas' : ['arkansas'],
                            'el gouna' : ['el gouna'],
                            'manaus' : ['manaus'],
                            'kythira' : ['kythira'],
                            'jericoacoara' : ['jericoacoara'],
                            'oklahoma' : ['oklahoma'],
                            'campania' : ['campania'],
                            'kimolos' : ['kimolos'],
                            'leh' : ['leh'],
                            'puno' : ['puno'],
                            'county kildare' : ['county kildare'],
                            'amazonas' : ['amazonas'],
                            'pushkar' : ['pushkar'],
                            'west virginia' : ['west virginia'],
                            'zurs' : ['zurs'],
                            'brasilia' : ['brasilia'],
                            'davos' : ['davos'],
                            'arezzo' : ['arezzo'],
                            'salta' : ['salta'],
                            'alentejo' : ['alentejo'],
                            'illinois' : ['illinois'],
                            'graz' : ['graz'],
                            'san juan islands' : ['san juan islands'],
                            'matsumoto' : ['matsumoto'],
                            'samoens' : ['samoens'],
                            'inverary' : ['inverary'],
                            'mougins' : ['mougins'],
                            'ponza' : ['ponza'],
                            'alexandria virginia' : ['alexandria virginia'],
                            'fornalutx' : ['fornalutx'],
                            'providenciales' : ['providenciales'],
                            'east hampton' : ['east hampton'],
                            'mystic' : ['mystic'],
                            'mulhouse' : ['mulhouse'],
                            'avila' : ['avila'],
                            'altura' : ['altura'],
                            'olhos de agua' : ['olhos de agua'],
                            'ragusa' : ['ragusa'],
                            'lancaster pa' : ['lancaster pa'],
                            'dawson' : ['dawson'],
                            'durham north carolina' : ['durham north carolina'],
                            'ferrara' : ['ferrara'],
                            'les deux alpes' : ['les deux alpes'],
                            'bayonne' : ['bayonne'],
                            'vernon' : ['vernon'],
                            'morgan bay' : ['morgan bay'],
                            'amelia island' : ['amelia island'],
                            'phuket island' : ['phuket island'],
                            'chinon' : ['chinon'],
                            'coorg' : ['coorg'],
                            'osijek' : ['osijek'],
                            'clermont ferrand' : ['clermont ferrand'],
                            'guwahati' : ['guwahati'],
                            'saskatoon' : ['saskatoon'],
                            'polignano a mare' : ['polignano a mare'],
                            'bonifacio' : ['bonifacio'],
                            'paternoster' : ['paternoster'],
                            'berchtesgaden' : ['berchtesgaden'],
                            'albi' : ['albi'],
                            'saskatchewan' : ['saskatchewan'],
                            'punta arenas' : ['punta arenas'],
                            'opatija' : ['opatija'],
                            'haputale' : ['haputale'],
                            'jalisco' : ['jalisco'],
                            'spa' : ['spa'],
                            'waiheke island' : ['waiheke island'],
                            'cantabria' : ['cantabria'],
                            'soll' : ['soll'],
                            'hakuba' : ['hakuba'],
                            'isla mujeres' : ['isla mujeres'],
                            'quimper' : ['quimper'],
                            'mdina' : ['mdina'],
                            'la paz' : ['la paz'],
                            'northland' : ['northland'],
                            'inverness shire' : ['inverness shire'],
                            'yukon' : ['yukon'],
                            'kythnos' : ['kythnos'],
                            'nyon' : ['nyon'],
                            'giritale' : ['giritale'],
                            'obidos' : ['obidos'],
                            'nagasaki' : ['nagasaki'],
                            'shillong' : ['shillong'],
                            'aosta valley' : ['aosta valley'],
                            'campeche' : ['campeche'],
                            'verdun' : ['verdun'],
                            'forte dei marmi' : ['forte dei marmi'],
                            'ladakh' : ['ladakh'],
                            'saalbach hinterglemm' : ['saalbach hinterglemm'],
                            'curitiba' : ['curitiba'],
                            'south australia' : ['south australia'],
                            'bhuj' : ['bhuj'],
                            'cortona' : ['cortona'],
                            'swellendam' : ['swellendam'],
                            'roatan' : ['roatan'],
                            'leros' : ['leros'],
                            'srinagar' : ['srinagar'],
                            'regensburg' : ['regensburg'],
                            'kastellorizo' : ['kastellorizo'],
                            'guanajuato' : ['guanajuato'],
                            'abruzzo' : ['abruzzo'],
                            'lofoten islands' : ['lofoten islands'],
                            'nizwa' : ['nizwa'],
                            'cirali' : ['cirali'],
                            'coonoor' : ['coonoor'],
                            'isle of anglesey' : ['isle of anglesey'],
                            'solden' : ['solden'],
                            'lake taupo' : ['lake taupo'],
                            'paraty' : ['paraty'],
                            'orvieto' : ['orvieto'],
                            'nebraska' : ['nebraska'],
                            'aeolian islands' : ['aeolian islands'],
                            'manitoba' : ['manitoba'],
                            'managua' : ['managua'],
                            'bacalar' : ['bacalar'],
                            'pag island' : ['pag island'],
                            'queretaro' : ['queretaro'],
                            'freiburg' : ['freiburg'],
                            'tioman' : ['tioman'],
                            'rudesheim' : ['rudesheim'],
                            'south tyrol' : ['south tyrol'],
                            'northern territory' : ['northern territory'],
                            'county galway' : ['county galway'],
                            'saas fee' : ['saas fee'],
                            'porto cervo' : ['porto cervo'],
                            'boothbay harbor' : ['boothbay harbor'],
                            'denbighshire' : ['denbighshire'],
                            'mendocino' : ['mendocino'],
                            'block island' : ['block island'],
                            'martinborough' : ['martinborough'],
                            'virgin gorda' : ['virgin gorda'],
                            'bandol' : ['bandol'],
                            'chur' : ['chur'],
                            'montreuil' : ['montreuil'],
                            'fredericton' : ['fredericton'],
                            'kemble' : ['kemble'],
                            'tubingen' : ['tubingen'],
                            'county fermanagh' : ['county fermanagh'],
                            'cahors' : ['cahors'],
                            'sainte maxime' : ['sainte maxime'],
                            'telluride' : ['telluride'],
                            'lipari' : ['lipari'],
                            'gallipoli' : ['gallipoli'],
                            'llanes' : ['llanes'],
                            'miyajima' : ['miyajima'],
                            'vichy' : ['vichy'],
                            'lynchburg' : ['lynchburg'],
                            'provo' : ['provo'],
                            'stanford' : ['stanford'],
                            'selcuk' : ['selcuk'],
                            'lanai' : ['lanai'],
                            'westendorf' : ['westendorf'],
                            'santo domingo' : ['santo domingo'],
                            'nevada' : ['nevada'],
                            'teton village' : ['teton village'],
                            'selva' : ['selva'],
                            'limone' : ['limone'],
                            'concord' : ['concord'],
                            'ashland' : ['ashland'],
                            'gros islet' : ['gros islet'],
                            'panglao island' : ['panglao island'],
                            'aquitaine' : ['aquitaine'],
                            'beaulieu sur mer' : ['beaulieu sur mer'],
                            'briancon' : ['briancon'],
                            'gordes' : ['gordes'],
                            'kashmir' : ['kashmir'],
                            'salo' : ['salo'],
                            'merano' : ['merano'],
                            'cernobbio' : ['cernobbio'],
                            'aurangabad' : ['aurangabad'],
                            'eleuthera' : ['eleuthera'],
                            'st remy de provence' : ['st remy de provence'],
                            'springdale' : ['springdale'],
                            'lenox' : ['lenox'],
                            'lens' : ['lens'],
                            'okayama' : ['okayama'],
                            'trier' : ['trier'],
                            'chiapas' : ['chiapas'],
                            'cali' : ['cali'],
                            'perigueux' : ['perigueux'],
                            'battambang' : ['battambang'],
                            'peniche' : ['peniche'],
                            'mannheim' : ['mannheim'],
                            'puerto maldonado' : ['puerto maldonado'],
                            'potsdam' : ['potsdam'],
                            'oberstdorf' : ['oberstdorf'],
                            'idaho' : ['idaho'],
                            'champery' : ['champery'],
                            'veracruz' : ['veracruz'],
                            'vang vieng' : ['vang vieng'],
                            'kep' : ['kep'],
                            'free state' : ['free state'],
                            'uyuni' : ['uyuni'],
                            'granada nicaragua' : ['granada nicaragua'],
                            'liguria' : ['liguria'],
                            'schladming' : ['schladming'],
                            'bamberg' : ['bamberg'],
                            'county wexford' : ['county wexford'],
                            'aguascalientes' : ['aguascalientes'],
                            'karnataka' : ['karnataka'],
                            'konstanz' : ['konstanz'],
                            'pelion' : ['pelion'],
                            'extremadura' : ['extremadura'],
                            'agrigento' : ['agrigento'],
                            'sendai' : ['sendai'],
                            'puerto iguazu' : ['puerto iguazu'],
                            'skyros' : ['skyros'],
                            'ollantaytambo' : ['ollantaytambo'],
                            'alsace' : ['alsace'],
                            'puerto natales' : ['puerto natales'],
                            'villars' : ['villars'],
                            'assam' : ['assam'],
                            'himachal pradesh' : ['himachal pradesh'],
                            'elba' : ['elba'],
                            'madhya pradesh' : ['madhya pradesh'],
                            'bikaner' : ['bikaner'],
                            'corvara' : ['corvara'],
                            'siargao island' : ['siargao island'],
                            'county leitrim' : ['county leitrim'],
                            'ceret' : ['ceret'],
                            'bastia' : ['bastia'],
                            'sint maarten' : ['sint maarten'],
                            'les carroz' : ['les carroz'],
                            'deerfield beach' : ['deerfield beach'],
                            'soufriere' : ['soufriere'],
                            'royan' : ['royan'],
                            'beaver creek' : ['beaver creek'],
                            'auxerre' : ['auxerre'],
                            'alpbach' : ['alpbach'],
                            'natchez' : ['natchez'],
                            'little cayman' : ['little cayman'],
                            'sawai madhopur' : ['sawai madhopur'],
                            'massa lubrense' : ['massa lubrense'],
                            'kasauli' : ['kasauli'],
                            'besancon' : ['besancon'],
                            'argeles sur mer' : ['argeles sur mer'],
                            'pelling' : ['pelling'],
                            'cumberland island' : ['cumberland island'],
                            'carmona' : ['carmona'],
                            'manacor' : ['manacor'],
                            'sestri levante' : ['sestri levante'],
                            'covington' : ['covington'],
                            'monticello' : ['monticello'],
                            'hondarribia' : ['hondarribia'],
                            'pinhao' : ['pinhao'],
                            'les menuires' : ['les menuires'],
                            'port antonio' : ['port antonio'],
                            'st peter ording' : ['st peter ording'],
                            'ojai' : ['ojai'],
                            'charlottetown' : ['charlottetown'],
                            'cayman brac' : ['cayman brac'],
                            'chablis' : ['chablis'],
                            'bhopal' : ['bhopal'],
                            'valence' : ['valence'],
                            'crosthwaite' : ['crosthwaite'],
                            'zacatecas' : ['zacatecas'],
                            'darmstadt' : ['darmstadt'],
                            'fernie' : ['fernie'],
                            'brandenburg' : ['brandenburg'],
                            'county sligo' : ['county sligo'],
                            'northern portugal' : ['northern portugal'],
                            'tyrol' : ['tyrol'],
                            'uttarakhand' : ['uttarakhand'],
                            'maharashtra' : ['maharashtra'],
                            'pangkor' : ['pangkor'],
                            'jammu and kashmir' : ['jammu and kashmir'],
                            'paarl' : ['paarl'],
                            'guayaquil' : ['guayaquil'],
                            'picton' : ['picton'],
                            'county monaghan' : ['county monaghan'],
                            'bloemfontein' : ['bloemfontein'],
                            'hakodate' : ['hakodate'],
                            'obertauern' : ['obertauern'],
                            'pas de calais' : ['pas de calais'],
                            'rothenburg ob der tauber' : ['rothenburg ob der tauber'],
                            'western cape' : ['western cape'],
                            'sounio' : ['sounio'],
                            'maratea' : ['maratea'],
                            'nancy' : ['nancy'],
                            'west bengal' : ['west bengal'],
                            'neuchatel' : ['neuchatel'],
                            'aragon' : ['aragon'],
                            'guanacaste' : ['guanacaste'],
                            'kiel' : ['kiel'],
                            'evia' : ['evia'],
                            'madonna di campiglio' : ['madonna di campiglio'],
                            'wiesbaden' : ['wiesbaden'],
                            'oberammergau' : ['oberammergau'],
                            'kwazulu natal' : ['kwazulu natal'],
                            'gottingen' : ['gottingen'],
                            'kaprun' : ['kaprun'],
                            'punta mita' : ['punta mita'],
                            'oaxaca city' : ['oaxaca city'],
                            'paracas' : ['paracas'],
                            'friedrichshafen' : ['friedrichshafen'],
                            'andermatt' : ['andermatt'],
                            'gujarat' : ['gujarat'],
                            'puerto varas' : ['puerto varas'],
                            'vejer de la frontera' : ['vejer de la frontera'],
                            'kottayam' : ['kottayam'],
                            'paro' : ['paro'],
                            'taunggyi' : ['taunggyi'],
                            'uttar pradesh' : ['uttar pradesh'],
                            'sucre' : ['sucre'],
                            'zwolle' : ['zwolle'],
                            'praiano' : ['praiano'],
                            'sancerre' : ['sancerre'],
                            'anjuna' : ['anjuna'],
                            'aude' : ['aude'],
                            'llucmajor' : ['llucmajor'],
                            'hossegor' : ['hossegor'],
                            'cat island' : ['cat island'],
                            'nashik' : ['nashik'],
                            'st croix' : ['st croix'],
                            'hoedspruit' : ['hoedspruit'],
                            'eureka springs' : ['eureka springs'],
                            'gardone riviera' : ['gardone riviera'],
                            'gwalior' : ['gwalior'],
                            'urgup' : ['urgup'],
                            'rafina' : ['rafina'],
                            'patiala' : ['patiala'],
                            'wolfsburg' : ['wolfsburg'],
                            'spoleto' : ['spoleto'],
                            'hyeres' : ['hyeres'],
                            'vannes' : ['vannes'],
                            'paestum' : ['paestum'],
                            'malelane' : ['malelane'],
                            'limon' : ['limon'],
                            'vieques' : ['vieques'],
                            'martina franca' : ['martina franca'],
                            'vienne' : ['vienne'],
                            'trujillo' : ['trujillo'],
                            'khajuraho' : ['khajuraho'],
                            'agen' : ['agen'],
                            'boca grande' : ['boca grande'],
                            'auvergne' : ['auvergne'],
                            'punakha' : ['punakha'],
                            'santa rosa beach' : ['santa rosa beach'],
                            'languedoc roussillon' : ['languedoc roussillon'],
                            'carmel valley' : ['carmel valley'],
                            'thimphu' : ['thimphu'],
                            'vitoria gasteiz' : ['vitoria gasteiz'],
                            'rocky harbour' : ['rocky harbour'],
                            'maiori' : ['maiori'],
                            'cochem' : ['cochem'],
                            'de panne' : ['de panne'],
                            'shelter island' : ['shelter island'],
                            'calistoga' : ['calistoga'],
                            'vico equense' : ['vico equense'],
                            'rocamadour' : ['rocamadour'],
                            'kandersteg' : ['kandersteg'],
                            'ninh binh' : ['ninh binh'],
                            'santa catarina' : ['santa catarina'],
                            'lindau' : ['lindau'],
                            'akaroa' : ['akaroa'],
                            'kumbhalgarh' : ['kumbhalgarh'],
                            'la rioja' : ['la rioja'],
                            'emilia romagna' : ['emilia romagna'],
                            'palenque' : ['palenque'],
                            'roermond' : ['roermond'],
                            'skagen' : ['skagen'],
                            'bahia' : ['bahia'],
                            'county limerick' : ['county limerick'],
                            'todi' : ['todi'],
                            'dordrecht' : ['dordrecht'],
                            'noto' : ['noto'],
                            'carinthia' : ['carinthia'],
                            'county laois' : ['county laois'],
                            'osnabruck' : ['osnabruck'],
                            'argentiere' : ['argentiere'],
                            'robertson' : ['robertson'],
                            'siwa' : ['siwa'],
                            'ystad' : ['ystad'],
                            'pingyao' : ['pingyao'],
                            'washington state' : ['washington state'],
                            'trancoso' : ['trancoso'],
                            'stepantsminda' : ['stepantsminda'],
                            'st johns' : ['st johns'],
                            'la baule' : ['la baule'],
                            'orchha' : ['orchha'],
                            'natal' : ['natal'],
                            'alappuzha' : ['alappuzha'],
                            'zihuatanejo' : ['zihuatanejo'],
                            'tamil nadu' : ['tamil nadu'],
                            'modica' : ['modica'],
                            'luneburg' : ['luneburg'],
                            'chefchaouen' : ['chefchaouen'],
                            'jeffreys bay' : ['jeffreys bay'],
                            'berat' : ['berat'],
                            'tangalle' : ['tangalle'],
                            'iquique' : ['iquique'],
                            'qui nhon' : ['qui nhon'],
                            'michoacan' : ['michoacan'],
                            'klosters' : ['klosters'],
                            'astypalaia' : ['astypalaia'],
                            'st wolfgang' : ['st wolfgang'],
                            'county kilkenny' : ['county kilkenny'],
                            'guerrero' : ['guerrero'],
                            'hokitika' : ['hokitika'],
                            'san pedro de atacama' : ['san pedro de atacama'],
                            'arosa' : ['arosa'],
                            'snowmass' : ['snowmass'],
                            'locarno' : ['locarno'],
                            'asuncion' : ['asuncion'],
                            'baqueira' : ['baqueira'],
                            'mpumalanga' : ['mpumalanga'],
                            'el calafate' : ['el calafate'],
                            'puntarenas' : ['puntarenas'],
                            'castres' : ['castres'],
                            'hawkes bay' : ['hawkes bay'],
                            'torri del benaco' : ['torri del benaco'],
                            'anissaras' : ['anissaras'],
                            'amherst' : ['amherst'],
                            'kota' : ['kota'],
                            'etretat' : ['etretat'],
                            'lunenburg' : ['lunenburg'],
                            'cambridge ontario' : ['cambridge ontario'],
                            'ubeda' : ['ubeda'],
                            'gaucin' : ['gaucin'],
                            'st simons island' : ['st simons island'],
                            'saintes' : ['saintes'],
                            'yucatan peninsula' : ['yucatan peninsula'],
                            'southport north carolina' : ['southport north carolina'],
                            'yakushima' : ['yakushima'],
                            'bad hofgastein' : ['bad hofgastein'],
                            'cluny' : ['cluny'],
                            'dalmatian coast' : ['dalmatian coast'],
                            'chikmagalur' : ['chikmagalur'],
                            'roquebrune cap martin' : ['roquebrune cap martin'],
                            'pistoia' : ['pistoia'],
                            'rhone alpes' : ['rhone alpes'],
                            'county armagh' : ['county armagh'],
                            'sainte foy tarentaise' : ['sainte foy tarentaise'],
                            'el hierro' : ['el hierro'],
                            'avranches' : ['avranches'],
                            'landshut' : ['landshut'],
                            'nord' : ['nord'],
                            'stillwater' : ['stillwater'],
                            'rhone alps' : ['rhone alps'],
                            'gulmarg' : ['gulmarg'],
                            'ile de france' : ['ile de france'],
                            'franche comte' : ['franche comte'],
                            'gansbaai' : ['gansbaai'],
                            'english harbour' : ['english harbour'],
                            'grand turk' : ['grand turk'],
                            'wuppertal' : ['wuppertal'],
                            'brig' : ['brig'],
                            'bad gastein' : ['bad gastein'],
                            'uzes' : ['uzes'],
                            'latina' : ['latina'],
                            'raipur' : ['raipur'],
                            'eymet' : ['eymet'],
                            'agde' : ['agde'],
                            'sandwich ma' : ['sandwich ma'],
                            'aosta' : ['aosta'],
                            'meursault' : ['meursault'],
                            'squaw valley' : ['squaw valley'],
                            'tiburon' : ['tiburon'],
                            'sag harbor' : ['sag harbor'],
                            'guarda' : ['guarda'],
                            'champoluc' : ['champoluc'],
                            'nagapattinam' : ['nagapattinam'],
                            'thun' : ['thun'],
                            'cagnes sur mer' : ['cagnes sur mer'],
                            'almora' : ['almora'],
                            'ross shire' : ['ross shire'],
                            'dawson city' : ['dawson city'],
                            'kakadu national park' : ['kakadu national park'],
                            'occidental' : ['occidental'],
                            'brantome' : ['brantome'],
                            'condom' : ['condom'],
                            'getaria' : ['getaria'],
                            'gengenbach' : ['gengenbach'],
                            'st gallen' : ['st gallen'],
                            'bundi' : ['bundi'],
                            'greensboro' : ['greensboro'],
                            'field' : ['field'],
                            'kalimpong' : ['kalimpong'],
                            'galaxidi' : ['galaxidi'],
                            'odisha' : ['odisha'],
                            'gramado' : ['gramado'],
                            'santa cruz de la sierra' : ['santa cruz de la sierra'],
                            'iraklia' : ['iraklia'],
                            'le chable' : ['le chable'],
                            'alpendorf' : ['alpendorf'],
                            'heron island' : ['heron island'],
                            'le marche' : ['le marche'],
                            'morro de sao paulo' : ['morro de sao paulo'],
                            'les diablerets' : ['les diablerets'],
                            'chiloe' : ['chiloe'],
                            'san juan del sur' : ['san juan del sur'],
                            'losinj' : ['losinj'],
                            'upper austria' : ['upper austria'],
                            'addo' : ['addo'],
                            'haute savoie' : ['haute savoie'],
                            'kicking horse' : ['kicking horse'],
                            'la ceiba' : ['la ceiba'],
                            'dungun' : ['dungun'],
                            'clervaux' : ['clervaux'],
                            'ito' : ['ito'],
                            'magdalen islands' : ['magdalen islands'],
                            'banos' : ['banos'],
                            'cote d azur' : ['cote d azur'],
                            'flims' : ['flims'],
                            'hoa binh' : ['hoa binh'],
                            'miyakojima' : ['miyakojima'],
                            'tepoztlan' : ['tepoztlan'],
                            'bad durkheim' : ['bad durkheim'],
                            'matakana' : ['matakana'],
                            'rugen' : ['rugen'],
                            'uvita' : ['uvita'],
                            'fernando de noronha' : ['fernando de noronha'],
                            'viana do castelo' : ['viana do castelo'],
                            'chubut' : ['chubut'],
                            'manizales' : ['manizales'],
                            'bregenz' : ['bregenz'],
                            'amrum' : ['amrum'],
                            'martigny' : ['martigny'],
                            'county westmeath' : ['county westmeath'],
                            'oualidia' : ['oualidia'],
                            'limousin' : ['limousin'],
                            'villa la angostura' : ['villa la angostura'],
                            'san cristobal de las casas' : ['san cristobal de las casas'],
                            'ometepe' : ['ometepe'],
                            'rhineland palatinate' : ['rhineland palatinate'],
                            'meissen' : ['meissen'],
                            'schinoussa' : ['schinoussa'],
                            'edam' : ['edam'],
                            'rodez' : ['rodez'],
                            'gorlitz' : ['gorlitz'],
                            'popayan' : ['popayan'],
                            'punta del este' : ['punta del este'],
                            'puducherry' : ['puducherry'],
                            'cotopaxi' : ['cotopaxi'],
                            'naxxar' : ['naxxar'],
                            'ko pha ngan' : ['ko pha ngan'],
                            'mancora' : ['mancora'],
                            'vorarlberg' : ['vorarlberg'],
                            'mirepoix' : ['mirepoix'],
                            'puerto madryn' : ['puerto madryn'],
                            'badenweiler' : ['badenweiler'],
                            'adelboden' : ['adelboden'],
                            'saarland' : ['saarland'],
                            'lot' : ['lot'],
                            'brissago' : ['brissago'],
                            'weggis' : ['weggis'],
                            'el chalten' : ['el chalten'],
                            'gard' : ['gard'],
                            'ruka' : ['ruka'],
                            'charente' : ['charente'],
                            'hesse' : ['hesse'],
                            'pontresina' : ['pontresina'],
                            'weimar' : ['weimar'],
                            'zell am ziller' : ['zell am ziller'],
                            'morelia' : ['morelia'],
                            'ceara' : ['ceara'],
                            'champasak' : ['champasak'],
                            'lot et garonne' : ['lot et garonne'],
                            'grimentz' : ['grimentz'],
                            'shekhawati' : ['shekhawati'],
                            'urubamba' : ['urubamba'],
                            'trentino' : ['trentino'],
                            'faaborg' : ['faaborg'],
                            'pereira' : ['pereira'],
                            'rostock' : ['rostock'],
                            'simons town' : ['simons town'],
                            'alacat' : ['alacat'],
                            'timaru' : ['timaru'],
                            'ootmarsum' : ['ootmarsum'],
                            'barichara' : ['barichara'],
                            'ellmau' : ['ellmau'],
                            'sylt' : ['sylt'],
                            'providencia' : ['providencia'],
                            'mcely' : ['mcely'],
                            'usedom' : ['usedom'],
                            'bayreuth' : ['bayreuth'],
                            'north rhine westphalia' : ['north rhine westphalia'],
                            'krk island' : ['krk island'],
                            'rab' : ['rab'],
                            'sipan' : ['sipan'],
                            'san martin de los andes' : ['san martin de los andes'],
                            'santa marta' : ['santa marta'],
                            'kyushu' : ['kyushu'],
                            'zanzibar town' : ['zanzibar town'],
                            'todos santos' : ['todos santos'],
                            'lower austria' : ['lower austria'],
                            'ubatuba' : ['ubatuba'],
                            'vendee' : ['vendee'],
                            'navarre' : ['navarre'],
                            'tegernsee' : ['tegernsee'],
                            'saxony' : ['saxony'],
                            'rio grande do sul' : ['rio grande do sul'],
                            'tabor' : ['tabor'],
                            'cyclades' : ['cyclades'],
                            'lenzerheide' : ['lenzerheide'],
                            'mato grosso' : ['mato grosso'],
                            'juist' : ['juist'],
                            'weissensee' : ['weissensee'],
                            'santarem' : ['santarem'],
                            'leersum' : ['leersum'],
                            'asti' : ['asti'],
                            'ilhabela' : ['ilhabela'],
                            'rosario' : ['rosario'],
                            'minas gerais' : ['minas gerais'],
                            'bornholm' : ['bornholm'],
                            'coburg' : ['coburg'],
                            'vaxjo' : ['vaxjo'],
                            'nikko' : ['nikko'],
                            'azay le rideau' : ['azay le rideau'],
                            'copiapo' : ['copiapo'],
                            'phu yen' : ['phu yen'],
                            'da lat' : ['da lat'],
                            'pernambuco' : ['pernambuco'],
                            'valle de bravo' : ['valle de bravo'],
                            'fnideq' : ['fnideq'],
                            'norderney' : ['norderney'],
                            'utila' : ['utila'],
                            'petropolis' : ['petropolis'],
                            'msambweni' : ['msambweni'],
                            'morgins' : ['morgins'],
                            'styria' : ['styria'],
                            'little corn island' : ['little corn island'],
                            'kea' : ['kea'],
                            'otago' : ['otago'],
                            'county waterford' : ['county waterford'],
                            'hintertux' : ['hintertux'],
                            'uchisar' : ['uchisar'],
                            'abcoude' : ['abcoude'],
                            'peten' : ['peten'],
                            'galtur' : ['galtur'],
                            'chenonceaux' : ['chenonceaux'],
                            'eastern cape' : ['eastern cape'],
                            'ouro preto' : ['ouro preto'],
                            'gironde' : ['gironde'],
                            'schleswig holstein' : ['schleswig holstein'],
                            'south island' : ['south island'],
                            'colonia del sacramento' : ['colonia del sacramento'],
                            'zebbug' : ['zebbug'],
                            'haute nendaz' : ['haute nendaz'],
                            'bra' : ['bra'],
                            'bastimentos' : ['bastimentos'],
                            'les sables dolonne' : ['les sables dolonne'],
                            'ardeche' : ['ardeche'],
                            'purmamarca' : ['purmamarca'],
                            'mafra' : ['mafra'],
                            'el jadida' : ['el jadida'],
                            'county carlow' : ['county carlow'],
                            'celerina' : ['celerina'],
                            'belo horizonte' : ['belo horizonte'],
                            'limpopo' : ['limpopo'],
                            'kaltenbach' : ['kaltenbach'],
                            'mar del plata' : ['mar del plata'],
                            'san miguel de allende' : ['san miguel de allende'],
                            'sandhamn' : ['sandhamn'],
                            'cuernavaca' : ['cuernavaca'],
                            'alajuela' : ['alajuela'],
                            'burgenland' : ['burgenland'],
                            'charente maritime' : ['charente maritime'],
                            'solta' : ['solta'],
                            'laax' : ['laax'],
                            'morges' : ['morges'],
                            'taroudant' : ['taroudant'],
                            'vitznau' : ['vitznau'],
                            'boca chica' : ['boca chica'],
                            'ascona' : ['ascona'],
                            'meghalaya' : ['meghalaya'],
                            'norden' : ['norden'],
                            'gotland' : ['gotland'],
                            'ionian islands' : ['ionian islands'],
                            'rio grande do norte' : ['rio grande do norte'],
                            'kristianstad' : ['kristianstad'],
                            'tanjore' : ['tanjore'],
                            'demodara' : ['demodara'],
                            'leukerbad' : ['leukerbad'],
                            'north island' : ['north island'],
                            'arica' : ['arica'],
                            'papallacta' : ['papallacta'],
                            'villa de leyva' : ['villa de leyva'],
                            'moissac' : ['moissac'],
                            'birgu' : ['birgu'],
                            'mystras' : ['mystras'],
                            'balapitiya' : ['balapitiya'],
                            'cardrona' : ['cardrona'],
                            'mecklenburg vorpommern' : ['mecklenburg vorpommern'],
                            'thuringia' : ['thuringia'],
                            'epirus' : ['epirus'],
                            'saarlouis' : ['saarlouis'],
                            'wellington western cape' : ['wellington western cape'],
                            'ilha grande' : ['ilha grande'],
                            'trakai' : ['trakai'],
                            'panajachel' : ['panajachel'],
                            'lizard island' : ['lizard island'],
                            'basque country spain' : ['basque country spain'],
                            'woerden' : ['woerden'],
                            'cafayate' : ['cafayate'],
                            'aguas calientes' : ['aguas calientes'],
                            'izu' : ['izu'],
                            'savoie' : ['savoie'],
                            'tilcara' : ['tilcara'],
                            'baden wurttemberg' : ['baden wurttemberg'],
                            'lower saxony' : ['lower saxony'],
                            'zao onsen' : ['zao onsen'],
                            'hiddensee' : ['hiddensee'],
                            'my tho' : ['my tho'],
                            'northern cape' : ['northern cape'],
                            'pruhonice' : ['pruhonice'],
                            'north west province' : ['north west province'],
                            'gondal' : ['gondal'],
                            'viseu' : ['viseu'],
                            'guethary' : ['guethary'],
                            'sasan gir' : ['sasan gir'],
                            'peschici' : ['peschici'],
                            'vogtsburg' : ['vogtsburg'],
                            'yvoire' : ['yvoire'],
                            'galatina' : ['galatina'],
                            'saguenay' : ['saguenay'],
                            'nuits saint georges' : ['nuits saint georges'],
                            'bad kissingen' : ['bad kissingen'],
                            'alberobello' : ['alberobello'],
                            'zapallar' : ['zapallar'],
                            'la trinite sur mer' : ['la trinite sur mer'],
                            'agropoli' : ['agropoli'],
                            'weeze' : ['weeze'],
                            'obernai' : ['obernai'],
                            'stavelot' : ['stavelot'],
                            'autun' : ['autun'],
                            'rendsburg' : ['rendsburg'],
                            'lyons la foret' : ['lyons la foret'],
                            'waimes' : ['waimes'],
                            'carhaix plouguer' : ['carhaix plouguer'],
                            'saint florent' : ['saint florent'],
                            'les eyzies' : ['les eyzies'],
                            'auch' : ['auch'],
                            'aquiraz' : ['aquiraz'],
                            'st johann in tirol' : ['st johann in tirol'],
                            'weil am rhein' : ['weil am rhein'],
                            'gargnano' : ['gargnano'],
                            'marsala' : ['marsala'],
                            'meersburg' : ['meersburg'],
                            'southport maine' : ['southport maine'],
                            'landes' : ['landes'],
                            'bad saarow' : ['bad saarow'],
                            'gustrow' : ['gustrow'],
                            'montignac' : ['montignac'],
                            'vieques island' : ['vieques island'],
                            'burg im spreewald' : ['burg im spreewald'],
                            'daun' : ['daun'],
                            'la roche bernard' : ['la roche bernard'],
                            'langeoog' : ['langeoog'],
                            'kuhlungsborn' : ['kuhlungsborn'],
                            'couiza' : ['couiza'],
                            'nerac' : ['nerac'],
                            'san vicente de la barquera' : ['san vicente de la barquera'],
                            'saint valery sur somme' : ['saint valery sur somme'],
                            'matsuyama' : ['matsuyama'],
                            'ehrwald' : ['ehrwald'],
                            'waldeck' : ['waldeck'],
                            'aups' : ['aups'],
                            'bozburun' : ['bozburun'],
                            'baru' : ['baru'],
                            'lodeve' : ['lodeve'],
                            'ponte de lima' : ['ponte de lima'],
                            'bad sooden allendorf' : ['bad sooden allendorf'],
                            'bhimtal' : ['bhimtal'],
                            'nardo' : ['nardo'],
                            'riobamba' : ['riobamba'],
                            'pontevedra' : ['pontevedra'],
                            'andros island' : ['andros island'],
                            'middlebury' : ['middlebury'],
                            'le boulou' : ['le boulou'],
                            'saint cyprien' : ['saint cyprien'],
                            'celle' : ['celle'],
                            'zuoz' : ['zuoz'],
                            'biscarrosse' : ['biscarrosse'],
                            'dunbartonshire' : ['dunbartonshire'],
                            'trebeurden' : ['trebeurden'],
                            'bella coola' : ['bella coola'],
                            'grobming' : ['grobming'],
                            'golling' : ['golling'],
                            'bevagna' : ['bevagna'],
                            'meze' : ['meze'],
                            'torres vedras' : ['torres vedras'],
                            'holderness' : ['holderness'],
                            'evian les bains' : ['evian les bains'],
                            'speyer' : ['speyer'],
                            'maglie' : ['maglie'],
                            'gerardmer' : ['gerardmer'],
                            'camaiore' : ['camaiore'],
                            'san felice del benaco' : ['san felice del benaco'],
                            'bex' : ['bex'],
                            'atami' : ['atami'],
                            'bad hersfeld' : ['bad hersfeld'],
                            'pieria' : ['pieria'],
                            'poligny' : ['poligny'],
                            'norcia' : ['norcia'],
                            'bas vully' : ['bas vully'],
                            'biella' : ['biella'],
                            'deidesheim' : ['deidesheim'],
                            'saintes maries de la mer' : ['saintes maries de la mer'],
                            'faralya' : ['faralya'],
                            'basilicata' : ['basilicata'],
                            'saint lo' : ['saint lo'],
                            'salers' : ['salers'],
                            'concon' : ['concon'],
                            'selestat' : ['selestat'],
                            'monthey' : ['monthey'],
                            'canet en roussillon' : ['canet en roussillon'],
                            'balduinstein' : ['balduinstein'],
                            'amarante' : ['amarante'],
                            'greenough' : ['greenough'],
                            'dobriach' : ['dobriach'],
                            'bad berleburg' : ['bad berleburg'],
                            'scuol' : ['scuol'],
                            'bad saulgau' : ['bad saulgau'],
                            'garberville' : ['garberville'],
                            'pordenone' : ['pordenone'],
                            'shahpura' : ['shahpura'],
                            'porto ercole' : ['porto ercole'],
                            'panarea' : ['panarea'],
                            'traben trarbach' : ['traben trarbach'],
                            'torno' : ['torno'],
                            'baiersbronn' : ['baiersbronn'],
                            'san vincenzo' : ['san vincenzo'],
                            'cabourg' : ['cabourg'],
                            'timmendorfer strand' : ['timmendorfer strand'],
                            'spitz' : ['spitz'],
                            'beckenried' : ['beckenried'],
                            'quedlinburg' : ['quedlinburg'],
                            'inhambane' : ['inhambane'],
                            'cayeux sur mer' : ['cayeux sur mer'],
                            'serra da estrela' : ['serra da estrela'],
                            'guingamp' : ['guingamp'],
                            'castelrotto' : ['castelrotto'],
                            'ortisei' : ['ortisei'],
                            'arco' : ['arco'],
                            'de haan' : ['de haan'],
                            'krems' : ['krems'],
                            'hinterthal' : ['hinterthal'],
                            'ka' : ['ka'],
                            'tux' : ['tux'],
                            'wagrain' : ['wagrain'],
                            'berwang' : ['berwang'],
                            'feldkirch' : ['feldkirch'],
                            'winterthur' : ['winterthur'],
                            'onomichi' : ['onomichi'],
                            'melchsee frutt' : ['melchsee frutt'],
                            'axamer lizum' : ['axamer lizum'],
                            'altenmarkt' : ['altenmarkt'],
                            'maragogi' : ['maragogi'],
                            'gubbio' : ['gubbio'],
                            'tarn' : ['tarn'],
                            'hittisau' : ['hittisau'],
                            'volkach' : ['volkach'],
                            'bad laasphe' : ['bad laasphe'],
                            'vals' : ['vals'],
                            'barneville carteret' : ['barneville carteret'],
                            'les crosets' : ['les crosets'],
                            'gernsbach' : ['gernsbach'],
                            'county londonderry' : ['county londonderry'],
                            'culebra' : ['culebra'],
                            'concarneau' : ['concarneau'],
                            'durnstein' : ['durnstein'],
                            'aigues mortes' : ['aigues mortes'],
                            'les trois vallees' : ['les trois vallees'],
                            'chieti' : ['chieti'],
                            'michelstadt' : ['michelstadt'],
                            'ahrenshoop' : ['ahrenshoop'],
                            'hawley' : ['hawley'],
                            'noci' : ['noci'],
                            'kaga' : ['kaga'],
                            'les andelys' : ['les andelys'],
                            'ottrott' : ['ottrott'],
                            'ciudad real' : ['ciudad real'],
                            'arrowtown' : ['arrowtown'],
                            'aurich' : ['aurich'],
                            'moustiers sainte marie' : ['moustiers sainte marie'],
                            'gaillac' : ['gaillac'],
                            'audierne' : ['audierne'],
                            'sartene' : ['sartene'],
                            'langenlois' : ['langenlois'],
                            'mende' : ['mende'],
                            'heiligendamm' : ['heiligendamm'],
                            'reggio calabria' : ['reggio calabria'],
                            'lons le saunier' : ['lons le saunier'],
                            'achenkirch' : ['achenkirch'],
                            'st andrews new brunswick' : ['st andrews new brunswick'],
                            'heiligenblut' : ['heiligenblut'],
                            'courtenay' : ['courtenay'],
                            'cannero riviera' : ['cannero riviera'],
                            'arta' : ['arta'],
                            'meiringen' : ['meiringen'],
                            'duderstadt' : ['duderstadt'],
                            'iruya' : ['iruya'],
                            'pakostane' : ['pakostane'],
                            'perros guirec' : ['perros guirec'],
                            'douarnenez' : ['douarnenez'],
                            'bouillon' : ['bouillon'],
                            'dahlonega' : ['dahlonega'],
                            'fayence' : ['fayence'],
                            'rockland' : ['rockland'],
                            'kabini river' : ['kabini river'],
                            'sabrosa' : ['sabrosa'],
                            'albarracin' : ['albarracin'],
                            'blankenburg' : ['blankenburg'],
                            'grignan' : ['grignan'],
                            'brand' : ['brand'],
                            'orient' : ['orient'],
                            'spiez' : ['spiez'],
                            'heavenly' : ['heavenly'],
                            'coyhaique' : ['coyhaique'],
                            'mont de marsan' : ['mont de marsan'],
                            'tournus' : ['tournus'],
                            'fulda' : ['fulda'],
                            'brigels' : ['brigels'],
                            'sanlucar de barrameda' : ['sanlucar de barrameda'],
                            'benasque' : ['benasque'],
                            'solothurn' : ['solothurn'],
                            'ramsau am dachstein' : ['ramsau am dachstein'],
                            'alcaniz' : ['alcaniz'],
                            'krun' : ['krun'],
                            'stralsund' : ['stralsund'],
                            'domme' : ['domme'],
                            'vals les bains' : ['vals les bains'],
                            'osuna' : ['osuna'],
                            'antalya province' : ['antalya province'],
                            'molitg les bains' : ['molitg les bains'],
                            'verneuil sur avre' : ['verneuil sur avre'],
                            'otavalo' : ['otavalo'],
                            'fasano' : ['fasano'],
                            'les baux de provence' : ['les baux de provence'],
                            'brail' : ['brail'],
                            'haltern am see' : ['haltern am see'],
                            'theoule sur mer' : ['theoule sur mer'],
                            'le lavandou' : ['le lavandou'],
                            'alcanar' : ['alcanar'],
                            'patzcuaro' : ['patzcuaro'],
                            'frankenberg' : ['frankenberg'],
                            'uberlingen' : ['uberlingen'],
                            'tahoe city' : ['tahoe city'],
                            'north adams' : ['north adams'],
                            'capdepera' : ['capdepera'],
                            'nyons' : ['nyons'],
                            'gorizia' : ['gorizia'],
                            'ainhoa' : ['ainhoa'],
                            'zahara de los atunes' : ['zahara de los atunes'],
                            'santona' : ['santona'],
                            'kressbronn am bodensee' : ['kressbronn am bodensee'],
                            'poperinge' : ['poperinge'],
                            'noboribetsu' : ['noboribetsu'],
                            'berea' : ['berea'],
                            'aracena' : ['aracena'],
                            'hluhluwe' : ['hluhluwe'],
                            'hinterzarten' : ['hinterzarten'],
                            'cogne' : ['cogne'],
                            'three forks' : ['three forks'],
                            'southwest harbor' : ['southwest harbor'],
                            'brive' : ['brive'],
                            'appenzell' : ['appenzell'],
                            'laglio' : ['laglio'],
                            'ladispoli' : ['ladispoli'],
                            'mulheim an der ruhr' : ['mulheim an der ruhr'],
                            'fassberg' : ['fassberg'],
                            'cancale' : ['cancale'],
                            'ecija' : ['ecija'],
                            'soria' : ['soria'],
                            'schwarzenberg' : ['schwarzenberg'],
                            'montrichard' : ['montrichard'],
                            'cassel' : ['cassel'],
                            'schwabisch hall' : ['schwabisch hall'],
                            'vezelay' : ['vezelay'],
                            'vila real' : ['vila real'],
                            'coffee bay' : ['coffee bay'],
                            'bodensdorf' : ['bodensdorf'],
                            'talloires' : ['talloires'],
                            'savelletri' : ['savelletri'],
                            'bad ragaz' : ['bad ragaz'],
                            'avallon' : ['avallon'],
                            'propriano' : ['propriano'],
                            'arcos de la frontera' : ['arcos de la frontera'],
                            'pinamar' : ['pinamar'],
                            'favignana' : ['favignana'],
                            'salt spring island' : ['salt spring island'],
                            'pont aven' : ['pont aven'],
                            'vulcano' : ['vulcano'],
                            'eltville' : ['eltville'],
                            'collias' : ['collias'],
                            'penafiel' : ['penafiel'],
                            'regensberg' : ['regensberg'],
                            'st luc' : ['st luc'],
                            'neustift' : ['neustift'],
                            'charmey' : ['charmey'],
                            'gramat' : ['gramat'],
                            'ribadesella' : ['ribadesella'],
                            'port alfred' : ['port alfred'],
                            'gauteng' : ['gauteng'],
                            'ramatuelle' : ['ramatuelle'],
                            'flachau' : ['flachau'],
                            'ourense' : ['ourense'],
                            'allariz' : ['allariz'],
                            'hirschegg' : ['hirschegg'],
                            'alpes maritimes' : ['alpes maritimes'],
                            'aveyron' : ['aveyron'],
                            'ribeauville' : ['ribeauville'],
                            'shima' : ['shima'],
                            'aviles' : ['aviles'],
                            'roanne' : ['roanne'],
                            'port carling' : ['port carling'],
                            'vrbnik' : ['vrbnik'],
                            'vila do conde' : ['vila do conde'],
                            'pauillac' : ['pauillac'],
                            'the basque country' : ['the basque country'],
                            'pfronten' : ['pfronten'],
                            'cesenatico' : ['cesenatico'],
                            'port camargue' : ['port camargue'],
                            'gourdon' : ['gourdon'],
                            'grossarl' : ['grossarl'],
                            'montichiari' : ['montichiari'],
                            'castellammare di stabia' : ['castellammare di stabia'],
                            'ranco' : ['ranco'],
                            'glucksburg' : ['glucksburg'],
                            'canyamel' : ['canyamel'],
                            'frutillar' : ['frutillar'],
                            'north hatley' : ['north hatley'],
                            'bonndorf' : ['bonndorf'],
                            'fujikawaguchiko' : ['fujikawaguchiko'],
                            'meisenheim' : ['meisenheim'],
                            'rayol canadel sur mer' : ['rayol canadel sur mer'],
                            'bentonville' : ['bentonville'],
                            'sanary sur mer' : ['sanary sur mer'],
                            'campos do jordao' : ['campos do jordao'],
                            'st jean pied de port' : ['st jean pied de port'],
                            'belle ile en mer' : ['belle ile en mer'],
                            'pont leveque' : ['pont leveque'],
                            'dinkelsbuhl' : ['dinkelsbuhl'],
                            'gumpoldskirchen' : ['gumpoldskirchen'],
                            'conca dei marini' : ['conca dei marini'],
                            'goslar' : ['goslar'],
                            'meerane' : ['meerane'],
                            'philipsburg' : ['philipsburg'],
                            'wellington ontario' : ['wellington ontario'],
                            'leogang' : ['leogang'],
                            'san vito dei normanni' : ['san vito dei normanni'],
                            'bridgehampton' : ['bridgehampton'],
                            'seillans' : ['seillans'],
                            'moselle' : ['moselle'],
                            'rottach egern' : ['rottach egern'],
                            'carantec' : ['carantec'],
                            'eglisau' : ['eglisau'],
                            'vaison la romaine' : ['vaison la romaine'],
                            'chateauneuf du pape' : ['chateauneuf du pape'],
                            'feldberg' : ['feldberg'],
                            'abaco islands' : ['abaco islands'],
                            'eisenach' : ['eisenach'],
                            'beaufort west' : ['beaufort west'],
                            'castellabate' : ['castellabate'],
                            'grimaud' : ['grimaud'],
                            'castelsardo' : ['castelsardo'],
                            'montefalco' : ['montefalco'],
                            'niort' : ['niort'],
                            'sierra de guadarrama' : ['sierra de guadarrama'],
                            'mulheim an der mosel' : ['mulheim an der mosel'],
                            'london on' : ['london on'],
                            'maheshwar' : ['maheshwar'],
                            'cuxhaven' : ['cuxhaven'],
                            'kanadukathan' : ['kanadukathan'],
                            'fiss' : ['fiss'],
                            'kirishima' : ['kirishima'],
                            'velden am worthersee' : ['velden am worthersee'],
                            'feuchtwangen' : ['feuchtwangen'],
                            'bad zwischenahn' : ['bad zwischenahn'],
                            'filzmoos' : ['filzmoos'],
                            'st gervais les bains' : ['st gervais les bains'],
                            'benodet' : ['benodet'],
                            'fieberbrunn' : ['fieberbrunn'],
                            'pesaro' : ['pesaro'],
                            'agua amarga' : ['agua amarga'],
                            'herault' : ['herault'],
                            'alpe dhuez' : ['alpe dhuez'],
                            'chagny' : ['chagny'],
                            'manteigas' : ['manteigas'],
                            'manchester village' : ['manchester village'],
                            'samnaun' : ['samnaun'],
                            'campos' : ['campos'],
                            'lahr' : ['lahr'],
                            'alquezar' : ['alquezar'],
                            'angra dos reis' : ['angra dos reis'],
                            'la morra' : ['la morra'],
                            'uzerche' : ['uzerche'],
                            'chintsa' : ['chintsa'],
                            'giulianova' : ['giulianova'],
                            'kusnacht' : ['kusnacht'],
                            'mandelieu la napoule' : ['mandelieu la napoule'],
                            'suffolk and essex' : ['suffolk and essex'],
                            'bad kleinkirchheim' : ['bad kleinkirchheim'],
                            'charolles' : ['charolles'],
                            'montauban' : ['montauban'],
                            'st christoph am arlberg' : ['st christoph am arlberg'],
                            'zweibrucken' : ['zweibrucken'],
                            'sommieres' : ['sommieres'],
                            'serre chevalier' : ['serre chevalier'],
                            'arzachena' : ['arzachena'],
                            'oberwesel' : ['oberwesel'],
                            'arromanches les bains' : ['arromanches les bains'],
                            'pringle bay' : ['pringle bay'],
                            'barrydale' : ['barrydale'],
                            'tarbes' : ['tarbes'],
                            'grosseto' : ['grosseto'],
                            'bietigheim bissingen' : ['bietigheim bissingen'],
                            'porreres' : ['porreres'],
                            'valpolicella' : ['valpolicella'],
                            'wolfville' : ['wolfville'],
                            'sare' : ['sare'],
                            'balderschwang' : ['balderschwang'],
                            'bonnieux' : ['bonnieux'],
                            'lezzeno' : ['lezzeno'],
                            'cordes sur ciel' : ['cordes sur ciel'],
                            'poitou charentes' : ['poitou charentes'],
                            'lasithi' : ['lasithi'],
                            'costa verde' : ['costa verde'],
                            'mirleft' : ['mirleft'],
                            'hautes pyrenees' : ['hautes pyrenees'],
                            'saxony anhalt' : ['saxony anhalt'],
                            'castile leon' : ['castile leon'],
                            'jardin' : ['jardin'],
                            'jujuy province' : ['jujuy province'],
                            'cospicua' : ['cospicua'],
                            'anapoima' : ['anapoima'],
                            'tandil' : ['tandil'],
                            'ansembourg' : ['ansembourg'],
                            'puerto misahualli' : ['puerto misahualli'],
                            'salta province' : ['salta province'],
                            'mendoza province' : ['mendoza province'],
                            'boquete' : ['boquete'],
                            'muhu island' : ['muhu island'],
                            'minakami' : ['minakami'],
                            'iwate' : ['iwate'],
                            'popoyo' : ['popoyo'],
                            'pichilemu' : ['pichilemu'],
                            'binh thun province' : ['binh thun province'],
                            'phu quc' : ['phu quc'],
                            'bad weissee' : ['bad weissee'],
                            'espirito santo' : ['espirito santo'],
                            'kalpitiya peninsula' : ['kalpitiya peninsula'],
                            'yanque' : ['yanque'],
                            'valledupar' : ['valledupar'],
                            'villa ohiggins' : ['villa ohiggins'],
                            'buritaca' : ['buritaca'],
                            'storms river' : ['storms river'],
                            'quimbaya' : ['quimbaya'],
                            'bedarra island' : ['bedarra island'],
                            'gers' : ['gers'],
                            'cachi' : ['cachi'],
                            'la barra' : ['la barra'],
                            'chimborazo province' : ['chimborazo province'],
                            'petrohue' : ['petrohue'],
                            'corrientes province' : ['corrientes province'],
                            'barra de sao miguel' : ['barra de sao miguel'],
                            'quang nam province' : ['quang nam province'],
                            'poli' : ['poli'],
                            'tatai' : ['tatai'],
                            'alagoas' : ['alagoas'],
                            'castile la mancha' : ['castile la mancha'],
                            'heredia province' : ['heredia province'],
                            'araucania region' : ['araucania region'],
                            'con dao islands' : ['con dao islands'],
                            'la pedrera' : ['la pedrera'],
                            'ljubno' : ['ljubno'],
                            'anhui province' : ['anhui province'],
                            'henne' : ['henne'],
                            'blokhus' : ['blokhus'],
                            'manabi' : ['manabi'],
                            'tisvilde' : ['tisvilde'],
                            'valencia region' : ['valencia region'],
                            'pyrenees orientales' : ['pyrenees orientales'],
                            'wanze' : ['wanze'],
                            'harads' : ['harads'],
                            'tasman' : ['tasman'],
                            'santa cruz province' : ['santa cruz province'],
                            'hokkaid' : ['hokkaid'],
                            'kladesholmen' : ['kladesholmen'],
                            'buenos aires province' : ['buenos aires province'],
                            'sao paulo state' : ['sao paulo state'],
                            'tierra bomba' : ['tierra bomba'],
                            'cordoba andalucia' : ['cordoba andalucia'],
                            'tribalj' : ['tribalj'],
                            'estremadura' : ['estremadura'],
                            'natures valley' : ['natures valley'],
                            'stawiguda' : ['stawiguda'],
                            'haarlen' : ['haarlen'],
                            'senglea' : ['senglea'],
                            'salzburg province' : ['salzburg province'],
                            'high atlas' : ['high atlas'],
                            'tehoum' : ['tehoum'],
                            'puerto guadal' : ['puerto guadal'],
                            'los lagos region' : ['los lagos region'],
                            'imbabura' : ['imbabura'],
                            'rio de janeiro state' : ['rio de janeiro state'],
                            'goias' : ['goias'],
                            'nong kiau' : ['nong kiau'],
                            'vreeland' : ['vreeland'],
                            'lustica bay' : ['lustica bay'],
                            'san antonio de areco' : ['san antonio de areco'],
                            'turkish aegean coast' : ['turkish aegean coast'],
                            'comala' : ['comala'],
                            'norddal' : ['norddal'],
                            'santa cruz la laguna' : ['santa cruz la laguna'],
                            'heggenes' : ['heggenes'],
                            'pyrenees atlantiques' : ['pyrenees atlantiques'],
                            'deqing' : ['deqing'],
                            'haggerstone island' : ['haggerstone island'],
                            'playa santana' : ['playa santana'],
                            'sveti klement island' : ['sveti klement island'],
                            'koveskal' : ['koveskal'],
                            'falkoping' : ['falkoping'],
                            'pipa beach' : ['pipa beach'],
                            'malalcahuello' : ['malalcahuello'],
                            'taboga' : ['taboga'],
                            'barreal' : ['barreal'],
                            'maule' : ['maule'],
                            'samaipata' : ['samaipata'],
                            'morahalom' : ['morahalom'],
                            'dodecanese' : ['dodecanese'],
                            'knislinge' : ['knislinge'],
                            'carmelo' : ['carmelo'],
                            'amantani island' : ['amantani island'],
                            'jarfalla' : ['jarfalla'],
                            'kawau island' : ['kawau island'],
                            'horve' : ['horve'],
                            'mompos' : ['mompos'],
                            'misiones' : ['misiones'],
                            'fohr' : ['fohr'],
                            'limon province' : ['limon province'],
                            'neuquen province' : ['neuquen province'],
                            'cordoba province' : ['cordoba province'],
                            'plitvica selo' : ['plitvica selo'],
                            'chateau doex' : ['chateau doex'],
                            'arraial dajuda' : ['arraial dajuda'],
                            'hautes alpes' : ['hautes alpes'],
                            'futaleufu' : ['futaleufu'],
                            'ile de porquerolles' : ['ile de porquerolles'],
                            'moutiers au perche' : ['moutiers au perche'],
                            'lisle sur la sorgue' : ['lisle sur la sorgue'],
                            'hauteluce' : ['hauteluce'],
                            'carlos pellegrini' : ['carlos pellegrini'],
                            'fincken' : ['fincken'],
                            'marfa' : ['marfa'],
                            'mailberg' : ['mailberg'],
                            'bernstein im burgenland' : ['bernstein im burgenland'],
                            'murbach' : ['murbach'],
                            'main brook' : ['main brook'],
                            'tarn et garonne' : ['tarn et garonne'],
                            'calolziocorte' : ['calolziocorte'],
                            'mollens' : ['mollens'],
                            'heinfels' : ['heinfels'],
                            'ile saint pierre' : ['ile saint pierre'],
                            'colombey les deux eglises' : ['colombey les deux eglises'],
                            'norris point' : ['norris point'],
                            'tobia' : ['tobia'],
                            'woodstock new york state' : ['woodstock new york'],
                            'ingapirca' : ['ingapirca'],
                            'los rios' : ['los rios'],
                            'schutzen am gebirge' : ['schutzen am gebirge'],
                            'napo province' : ['napo province'],
                            'lozere' : ['lozere'],
                            'autre eglise' : ['autre eglise'],
                            'puylaurens' : ['puylaurens'],
                            'castelo rodrigo' : ['castelo rodrigo'],
                            'le noirmont' : ['le noirmont'],
                            'calitzdorp' : ['calitzdorp'],
                            'plougrescant' : ['plougrescant'],
                            'brassac' : ['brassac'],
                            'lumio' : ['lumio'],
                            'neukloster' : ['neukloster'],
                            'bernalda' : ['bernalda'],
                            'val doise' : ['val doise'],
                            'duingt' : ['duingt'],
                            'best hotels in' : ['best hotels in'],
                            'koh kong province' : ['koh kong province'],
                            'cotacachi' : ['cotacachi'],
                            'deux sevres' : ['deux sevres'],
                            'genappe' : ['genappe'],
                            'herepian' : ['herepian'],
                            'gostling' : ['gostling'],
                            'southampton new york stat' : ['southampton new york'],
                            'nobsa' : ['nobsa'],
                            'hippach schwendau' : ['hippach schwendau'],
                            'ile de bendor' : ['ile de bendor'],
                            'sauternes' : ['sauternes'],
                            'frankenau' : ['frankenau'],
                            'quirpon island' : ['quirpon island'],
                            'lac la hache' : ['lac la hache'],
                            'pirenopolis' : ['pirenopolis'],
                            'zahora' : ['zahora'],
                            'east kemptville' : ['east kemptville'],
                            'peillon' : ['peillon'],
                            'negros island' : ['negros island'],
                            'ennetburgen' : ['ennetburgen'],
                            'lubben' : ['lubben'],
                            'sant miquel de balansat' : ['sant miquel de balansat'],
                            'kittendorf' : ['kittendorf'],
                            'lake bracciano' : ['lake bracciano'],
                            'bourdeau' : ['bourdeau'],
                            'albareto' : ['albareto'],
                            'gdask' : ['gdask'],
                            'inuyama' : ['inuyama'],
                            'cala sant vincenc' : ['cala sant vincenc'],
                            'penne dagenais' : ['penne dagenais'],
                            'constitucion' : ['constitucion'],
                            'el bierzo' : ['el bierzo'],
                            'unzen' : ['unzen'],
                            'sebersdorf' : ['sebersdorf'],
                            'tomonoura' : ['tomonoura'],
                            'rga' : ['rga'],
                            'channel ports' : ['channel ports'],
                            'ludorf' : ['ludorf'],
                            'ariege' : ['ariege'],
                            'montesquiou' : ['montesquiou'],
                            'ile doleron' : ['ile doleron'],
                            'seckau' : ['seckau'],
                            'ile de noirmoutier' : ['ile de noirmoutier'],
                            'lectoure' : ['lectoure'],
                            'sucumbios province' : ['sucumbios province'],
                            'highlands islands' : ['highlands islands'],
                            'pisco elqui' : ['pisco elqui'],
                            'saint guilhem le desert' : ['saint guilhem le desert'],
                            'general guemes' : ['general guemes'],
                            'ramallo' : ['ramallo'],
                            'garzon' : ['garzon'],
                            'wartmannsroth' : ['wartmannsroth'],
                            'pronstorf' : ['pronstorf'],
                            'cucugnan' : ['cucugnan'],
                            'phalsbourg' : ['phalsbourg'],
                            'geestland' : ['geestland'],
                            'vouzela' : ['vouzela'],
                            'blainville sur mer' : ['blainville sur mer'],
                            'saint laurent des arbres' : ['saint laurent des arbres'],
                            'a guarda' : ['a guarda'],
                            'cantal' : ['cantal'],
                            'san casciano in val di pesa' : ['san casciano in val di pesa'],
                            'vidago' : ['vidago'],
                            'minamiuonuma' : ['minamiuonuma'],
                            'lalbenque' : ['lalbenque'],
                            'port dandratx' : ['port dandratx'],
                            'hohen demzin' : ['hohen demzin'],
                            'ibarra' : ['ibarra'],
                            'afiesl' : ['afiesl'],
                            'sporades' : ['sporades'],
                            'puerto libertad' : ['puerto libertad'],
                            'haute loire' : ['haute loire'],
                            'tarascon sur ariege' : ['tarascon sur ariege'],
                            'villereal' : ['villereal'],
                            'highlands and islands' : ['highlands and islands'],
                            'montcuq' : ['montcuq'],
                            'madrid region' : ['madrid region'],
                            'puyuhuapi' : ['puyuhuapi'],
                            'fontevraud labbaye' : ['fontevraud labbaye'],
                            'grisons' : ['grisons'],
                            'le bar sur loup' : ['le bar sur loup'],
                            'jezersko' : ['jezersko'],
                            'haute garonne' : ['haute garonne'],
                            'sobernheim' : ['sobernheim'],
                            'tortel' : ['tortel'],
                            'hermance' : ['hermance'],
                            'peso da regua' : ['peso da regua'],
                            'carminha' : ['carminha'],
                            'isselburg' : ['isselburg'],
                            'chambolle musigny' : ['chambolle musigny'],
                            'puymirol' : ['puymirol'],
                            'tschiertschen' : ['tschiertschen'],
                            'toyooka' : ['toyooka'],
                            'saint pol de leon' : ['saint pol de leon'],
                            'madulain' : ['madulain'],
                            'setoda' : ['setoda'],
                            'madarao' : ['madarao'],
                            'la palud sur verdon' : ['la palud sur verdon'],
                            'felsberg' : ['felsberg'],
                            'sao paulo and south' : ['sao paulo and south'],
                            'ille sur tet' : ['ille sur tet'],
                            'malchow' : ['malchow'],
                            'nagahama' : ['nagahama'],
                            'britiande' : ['britiande'],
                            'matarrana' : ['matarrana'],
                            'fesfez' : ['fesfez'],
                            'capestang' : ['capestang'],
                            'siglufjorder' : ['siglufjorder'],
                            'blevio' : ['blevio'],
                            'locquirec' : ['locquirec'],
                            'pichincha province' : ['pichincha province'],
                            'marettimo' : ['marettimo'],
                            'le conquet' : ['le conquet'],
                            'bourbon larchambault' : ['bourbon larchambault'],
                            'gordevio' : ['gordevio'],
                            'ehrenhausen' : ['ehrenhausen'],
                            'malgrate' : ['malgrate'],
                            'llifen' : ['llifen'],
                            'castelnau de medoc' : ['castelnau de medoc'],
                            'sonnenbuhl' : ['sonnenbuhl'],
                            'ile daix' : ['ile daix'],
                            'bilsen' : ['bilsen'],
                            'koh russey' : ['koh russey'],
                            'hornbach' : ['hornbach'],
                            'victoria arg' : ['victoria arg'],
                            'bourdeilles' : ['bourdeilles'],
                            'korula' : ['korula'],
                            'douro valley' : ['douro valley'],
                            'basque country' : ['basque country'],
                            'south and mid wales' : ['south and mid wales'],
                            'sao tome' : ['sao tome'],
                            'mid wales' : ['mid wales'],
                            'south wales' : ['south wales'],
                            'bosnia' : ['bosnia'],
                            'herzegovina' : ['herzegovina'],
                            'turks' : ['turks'],
                            'the grenadines' : ['the grenadines'],
                            'st vincent' : ['st vincent'],
                            'moutiers' : ['moutiers'],
                            'european' : ['european'],
                            'europe' : ['europe'],
                            'asia' : ['asia'],
                            'africa' : ['africa'],
                            'south america' : ['south america'],
                            'usa' : ['usa'],
                            'america' : ['america'],
                            'england' : ['england'],
                            'uk' : ['uk'],
                            'britain' : ['britain'],
                            'new forest' : ['new forest'],
                            'cote d ivoire' : ['cote d ivoire']
                            }
            
    }

        # 'informational' : {'when to go' : ['when to go'], 'when' : ['when'], 'where to stay' : ['where to stay'], 'what is' : ['what is'], 'which part' : ['which part'], 'where to go' : ['where to go'], 'how' : ['how many', 'how much', 'how to'], 'things to do' : ['things to do', 'must see', 'things to see', 'what to do', 'whats on'], 'best time to' : ['best time to']},